<?php
	...
	$sql_where = implode(" and ",$sql_where);
     ...
     $sql = "select a.fol_id, a.id_reg, a.fol_nama ,c.cust_usr_nama, c.cust_usr_kode, a.fol_dibayar, b.biaya_nama, a.fol_jenis, a.id_biaya_tambahan,
	...
	for($i=0,$n=count($dataSplit);$i<$n;$i++){
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["fol_nama"]?$dataTable[$i]["fol_nama"]:$namaFolio[$dataTable[$i]["fol_jenis"]];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
          $counter++;
	  ...
	  for($j=0,$k=count($dataSplit);$j<$k;$j++){
	       ...
	       $totSprit[$dataTable[$i]["fol_id"]] += $dataFolSplit[$dataTable[$i]["fol_id"]][$dataSplit[$j]["split_id"]];
	  }


	  // if($dataTable[$i]["id_reg"]!=$dataTable[$i-1]["id_reg"]){
	  $tbContent[$i][$counter][TABLE_ISI] = currency_format($totSprit[$dataTable[$i]["fol_id"]]);
	  $tbContent[$i][$counter][TABLE_ALIGN] = "right";
	  // $tbContent[$i][$counter][ROWSPAN] = $dataSpan["jml_span"];
	  $counter++;
	  //     }
     ...
     $tbBottom[0][$counter][TABLE_STYLE] = "subheader";
     $counter++;
