<?php
    ...
    if($_GET["id_jab"]) $_POST["id_jab"] = $_GET["id_jab"];
    ...
    $addPage = "jab_act_add.php?id_jab=".$_POST["id_jab"];
    ...
    if ($_GET["id"]) {
        if ($_POST["btnDelete"]) { 
            ...
            $_x_mode = "Edit";
            $_POST["id_act"] = $_GET["id"];
        }
        $sql = "select a.*,b.jab_nama, c.act_tampil from sek_jabatan_action a join sek_jabatan b on a.id_jab = b.jab_id join sek_action c on a.id_act = c.act_id where a.id_act = ".$_POST["id_act"]." and a.id_jab = ".$_POST["id_jab"];
    ...
    if ($_POST["btnUpdate"]) {
        ...
        if ($err_code == 0) {
            ...
            header("location:jab_act_view.php?id=".$_POST["id_jab"]);
            exit();        
        }
    }
    ...
    if ($_POST["btnDelete"]) {
        ...
        for($i=0,$n=count($actId);$i<$n;$i++){
            $sql = "delete from sek_jabatan_action where id_act = ".$actId[$i]." and id_jab = ".$_POST["id_jab"];
...
<link rel="stylesheet" type="text/css" href="<?php echo $APLICATION_ROOT;?>library/css/expose.css">
