<?php
	...
	$table = new InoTable("table1","100%","center");
     ...
     if($rolId){
          ...
          $sql = "select distinct(c.priv_name),d.app_nama, d.app_id, a.*, b.rol_name from global.global_auth_role_priv a
	...
	if($dataTable[$i]["app_id"]!=$dataTable[$i-1]["app_id"]){
      $tbContent[$j][$counter][TABLE_ISI] = $dataTable[$i]["app_nama"];
      ...
      $j++;
		...
		if($auth->IsAllowed("setup_role",PRIV_DELETE)) {
			$tbContent[$j][$counter][TABLE_ISI] = '<input type="checkbox" name="cbDelete[]" value="'.$dataTable[$i]["id_priv"].'">';
			$tbContent[$j][$counter][TABLE_ALIGN] = "center";
			$counter++;
		}
		
		if($auth->IsAllowed("setup_role",PRIV_UPDATE)) {
			$tbContent[$j][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$dataTable[$i]["id_priv"].'"><img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/b_edit.png" alt="Edit" title="Edit" border="0"></a>';
			$tbContent[$j][$counter][TABLE_ALIGN] = "center";
			$counter++;
		}
		
          $tbContent[$j][$counter][TABLE_ISI] = $dataTable[$i]["priv_name"];
          $tbContent[$j][$counter][TABLE_ALIGN] = "left";
          $counter++;
          
          for($k=(7-4);$k<7;$k++) {
               $tbContent[$j][$counter][TABLE_ISI] = $logoPriv[$dataTable[$i]["rol_priv_access"]{$k-3}];
               $tbContent[$j][$counter][TABLE_ALIGN] = "center";
               $counter++;
          }        
	}
