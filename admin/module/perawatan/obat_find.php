...
function GetData($in_nama,$in_kode){
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["item_id"].'\',\''.$dataTable[$i]["item_fisik"].'\',\''.addslashes(htmlentities($dataTable[$i]["item_nama"])).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,fisik,nama) {
	...
	self.parent.SetDosis(fisik,'<?php echo $_GET["el"];?>');
