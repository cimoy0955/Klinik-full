...
function GetData($in_nama){
	...
	$sql_where[] = "1=1"; 
	if($in_nama) $sql_where[] = "UPPER(suster_nama) like ".QuoteValue(DPE_CHAR,strtoupper("%".$in_nama."%"));
	$sql_where = implode(" and ",$sql_where);

	// --- cari data krsnya ---
	$sql = "select suster_nama from global.global_auth_poli a";
	$sql .= " where ".$sql_where;			
	$sql .= " order by a.poli_nama";
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$dataTable[$i]["suster_nama"];
		$tbContent[$i][$counter][TABLE_ALIGN] = "left";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
		
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'gambar/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["poli_id"].'\',\''.addslashes(htmlentities($dataTable[$i]["suster_nama"])).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,nama) {
	...
	self.parent.document.getElementById('rawat_suster_nama_<?php echo $_GET["el"];?>').value = nama;
	self.parent.document.getElementById('id_suster_<?php echo $_GET["el"];?>').value = id;
