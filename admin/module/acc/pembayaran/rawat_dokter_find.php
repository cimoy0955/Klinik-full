...
function GetData($in_nama){
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'gambar/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["pgw_id"].'\',\''.addslashes(htmlentities($dataTable[$i]["pgw_nama"])).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,nama) {
	...
	self.parent.document.getElementById('rawat_dokter_nama').value = nama;
	self.parent.document.getElementById('id_dokter').value = id;
