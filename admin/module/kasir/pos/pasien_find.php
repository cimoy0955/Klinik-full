...
function GetData($in_nama,$in_kode,$in_alamat,$in_umur,$in_jk){
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["cust_usr_id"].'\',\''.$dataTable[$i]["cust_usr_nama"].'\')"/>';
...
function sendValue(id,nama) {
	self.parent.document.getElementById('id_reg').value = id;
	self.parent.document.getElementById('pasien_nama').value = nama;
