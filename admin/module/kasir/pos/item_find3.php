...
function GetData($in_nama=null){
	...
	if($in_nama) $sql_where .= " and UPPER(obat_nama) like '%".strtoupper($in_nama)."%'"; 

	$sql = "select * from apotik_obat_master ";
	if($sql_where){
	$sql .= $sql_where;
  }			
  ...
  $tbHeader[0][$counter][TABLE_ISI] = "Kode";
	...
	*/
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$counter++;
		/*
    $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$dataTable[$i]["obat_kode"];
		...
		*/
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.addslashes(htmlentities($dataTable[$i]["obat_nama"])).'\',\''.$dataTable[$i]["obat_id"].'\',\''.$dataTable[$i]["obat_harga_jual"].'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(nama,id,harga) {
	self.parent.document.getElementById('obat_nama_2').value = nama;
	self.parent.document.getElementById('obat_id_2').value = id;
	self.parent.document.getElementById('txtHargaSatuan_2').value = formatCurrency(harga);
	self.parent.document.getElementById('txtHargaTotal_2').value = formatCurrency(harga);
	self.parent.document.getElementById('txtJumlah_2').value = '1';
	self.parent.document.getElementById('txtJumlah_2').focus();
