	...
	*/
    ...
    $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$dataTable[$i]["obat_kode"];
		...
		*/
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.addslashes(htmlentities($dataTable[$i]["obat_nama"])).'\',\''.$dataTable[$i]["obat_id"].'\',\''.$dataTable[$i]["obat_stok"].'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(nama,id,stok) {
	self.parent.document.getElementById('opname_item_nama').value = nama;
	self.parent.document.getElementById('id_item').value = id;
	//self.parent.document.getElementById('obat_kode').value = kode;
	self.parent.document.getElementById('opname_stok_catat').value = formatCurrency(stok);
	self.parent.document.getElementById('opname_stok_real').focus();
