...
function GetData($in_nama=null){
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.addslashes(htmlentities($dataTable[$i]["item_nama"])).'\',\''.$dataTable[$i]["item_id"].'\',\''.currency_format($dataTable[$i]["item_harga_jual"]).'\',\''.currency_format($dataTable[$i]["item_harga_beli"]).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(nama,id,harga,hargaBeli) {
	self.parent.document.getElementById('item_nama').value = nama;
	self.parent.document.getElementById('item_id').value = id;
	self.parent.document.getElementById('item_harga_jual').value = harga;
	self.parent.document.getElementById('item_harga_beli').value = hargaBeli;
	self.parent.document.getElementById('transaksi_harga_beli').focus();
