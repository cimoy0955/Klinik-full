<?php
     ...
     $thisPage = "report_penyakit_diutamakan.php";
     ...
     if($_POST["tgl_akhir"]) $sql_where[] = "b.rawat_tanggal <= ".QuoteValue(DPE_DATE,date_db($_POST["tgl_akhir"]));
      ...
      $sql = "select * from global.global_penyakit order by penyakit_urut asc";
     ...
     $dataTable = $dtaccess->FetchAll($rs);
      ...
      $sql = "select count(a.reg_id) as total
            ...
            left join global.global_penyakit d on d.id_icd = c.id_icd ";
       $sql .="where a.reg_status_pasien = 'B' and d.id_icd = ".QuoteValue(DPE_CHAR,$dataTable[$i]["id_icd"])." and ".implode(" and ",$sql_where); 
       $rs = $dtaccess->Execute($sql);
     ...
     $sql = "select count(a.reg_id) as total
            ...
            left join klinik.klinik_perawatan_icd c on b.rawat_id = c.id_rawat
            left join global.global_penyakit d on d.id_icd = c.id_icd ";
       $sql .="where a.reg_status_pasien = 'L' and d.id_icd = ".QuoteValue(DPE_CHAR,$dataTable[$i]["id_icd"])." and ".implode(" and ",$sql_where);
       $rs = $dtaccess->Execute($sql);
       ...
       $icdLama[] = $dataLama['total'];
        ...
        $sql = "select count(a.reg_id) as total
            ...
            left join klinik.klinik_perawatan_icd c on b.rawat_id = c.id_rawat
            left join global.global_penyakit d on d.id_icd = c.id_icd ";
       $sql .="where d.id_icd = ".QuoteValue(DPE_CHAR,$dataTable[$i]["id_icd"])." and ".implode(" and ",$sql_where);
       $rs = $dtaccess->Execute($sql);
    ...
    // echo $sql;
     //*-- config table ---*//
     $tableHeader = "&nbsp;Report 10 Penyakit Diutamakan";
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["nomor_icd"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
          $counter++;
          
          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["nama_icd"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";          
          $counter++;
	...
	if($_POST["btnExcel"]){
          ...
          header('Content-Disposition: attachment; filename=report_penyakit_diutamakan.xls');
     }
...
function CheckSimpan() {
     if(confirm('Cetak Invoice?')) BukaWindow('cetak_10_penyakit.php?tgl_awal=<?php echo date_db($_POST["tgl_awal"]);?>&tgl_akhir=<?php echo date_db($_POST["tgl_akhir"]);?>','Invoice');
     return true;
}
