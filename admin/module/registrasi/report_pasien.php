<?php
     ...
     if($_POST["tgl_awal"]) $sql_where[] = "CAST(a.reg_tanggal as DATE) >= ".QuoteValue(DPE_DATE,date_db($_POST["tgl_awal"]));
     if($_POST["tgl_akhir"]) $sql_where[] = "CAST(a.reg_tanggal as DATE) <= ".QuoteValue(DPE_DATE,date_db($_POST["tgl_akhir"]));

     if($_POST["jenisKelamin"]&&$_POST["jenisKelamin"]!="-") $sql_where[] = "b.cust_usr_jenis_kelamin = ".QuoteValue(DPE_CHAR,$_POST["jenisKelamin"]);

     $sql = "select b.cust_usr_kode, b.cust_usr_nama, b.cust_usr_alamat, b.cust_usr_tanggal_lahir, b.cust_usr_jenis_kelamin, 
               a.reg_jenis_pasien, a.reg_status_pasien, a.reg_keterangan, a.reg_waktu, a.reg_tanggal   
	...
	if($_POST["btnExcel"]){
     ...
     $optKelamin[] = $view->RenderOption("-","[semua]",($_POST["jenisKelamin"]=="-")?"selected":"");
     foreach($jenisKelamin as $key => $value){
          $optKelamin[] = $view->RenderOption($key,$value,($key==$_POST["jenisKelamin"])?"selected":"");
     }
...
<table align="center" border=0 cellpadding=2 cellspacing=1 width="100%" class="tblForm" id="tblSearching">
     <tr>
          ...
          <tr>
          <td width="15%" class="tablecontent">&nbsp;Jenis Kelamin</td>
          <td width="25%" class="tablecontent-odd">
			<?php echo $view->RenderComboBox("jenisKelamin","jenisKelamin",$optKelamin,"inputfield",null,null);?>
          </td>
           </tr>
           <tr>
