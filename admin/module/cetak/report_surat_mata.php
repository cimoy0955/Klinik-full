<?php
     ...
     if($_POST["tgl_awal"]) $sql_where[] = "cast(surat_mata_when as date) >= ".QuoteValue(DPE_DATE,date_db($_POST["tgl_awal"]));  
     if($_POST["tgl_akhir"]) $sql_where[] = "cast(surat_mata_when as date) <= ".QuoteValue(DPE_DATE,date_db($_POST["tgl_akhir"]));  

     $sql = "select b.cust_usr_kode, b.cust_usr_nama, cust_usr_tanggal_lahir, a.*, pgw_nama,cast(surat_mata_when as date) as tanggal
     ...
     $sql.= " order by surat_mata_when, cust_usr_kode"; 
     $rs = $dtaccess->Execute($sql);
     $dataTable = $dtaccess->FetchAll($rs);
     
     //*-- config table ---*//
     $tableHeader = "&nbsp;Report Surat Kesehatan Mata"; 
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){ 
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["surat_mata_diagnosis"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";          
          $counter++;
	...
	if($_POST["btnExcel"]){
          ...
          header('Content-Disposition: attachment; filename=report_surat_mata_'.$_POST["tgl_awal"].'.xls');
     }
