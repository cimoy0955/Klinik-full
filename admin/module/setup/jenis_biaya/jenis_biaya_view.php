<?php
     ...
     $sql = "select a.* from klinik.klinik_split a 
               order by a.split_flag, a.split_nama ";
     $rs = $dtaccess->Execute($sql,DB_SCHEMA_GLOBAL);
     $dataTable = $dtaccess->FetchAll($rs);
     
     //*-- config table ---*//
     $tableHeader = "&nbsp;Setup Jenis Biaya";
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "Split";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "30%";
     $counterHeader++; 
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
         ...
         $counter++;
          ...
          if($isAllowedDel) {
               $tbContent[$i][$counter][TABLE_ISI] = '<input type="checkbox" name="cbDelete[]" value="'.$dataTable[$i]["split_id"].'">';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
          }
          
          if($isAllowedUpdate) {
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'?id='.$enc->Encode($dataTable[$i]["split_id"]).'"><img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/b_edit.png" alt="Edit" title="Edit" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
          }
          ...
          $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;&nbsp;".$namaSplit[$dataTable[$i]["split_flag"]];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
          $counter++; 
     }
