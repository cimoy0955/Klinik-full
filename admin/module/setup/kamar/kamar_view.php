<?php
     ...
     if(isset($_POST["btnSearch"]) || $cari || ($_GET["_search"]=="yes")) {
          ...
          $sql = "select a.*,b.biaya_nama from klinik.klinik_kamar a 
                    left join klinik.klinik_biaya b on b.biaya_jenis = a.id_kategori"; 
          if($sql_where){
     ...
     $sql = "select biaya_id,biaya_nama from klinik.klinik_biaya where biaya_jenis like '%K%'
               order by biaya_id";
     $rs = $dtaccess->Execute($sql);
     ...
     for($i=0,$n=count($datakategori);$i<$n;$i++) {
          unset($show);
          if($_POST["id_kategori"]==$datakategori[$i]["biaya_id"]) $show = "selected";
          $kategori[$i+1] = $view->RenderOption($datakategori[$i]["biaya_id"],$datakategori[$i]["biaya_nama"],$show);
     }
