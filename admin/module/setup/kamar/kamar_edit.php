<?php
	...
	elseif($_x_mode=="Edit") $privMode = PRIV_UPDATE;
     ...
     $sql = "select biaya_jenis,biaya_nama from klinik.klinik_biaya where biaya_jenis like '%K%'
               order by biaya_jenis";
     $rs = $dtaccess->Execute($sql);
     $datakategori = $dtaccess->FetchAll($rs);

     for($i=0,$n=count($datakategori);$i<$n;$i++) {
          unset($show);
          if($_POST["kamar_kategori"]==$datakategori[$i]["biaya_jenis"]) $show = "selected";
          $kategori[$i] = $view->RenderOption($datakategori[$i]["biaya_jenis"],$datakategori[$i]["biaya_nama"],$show);
     }
?>
