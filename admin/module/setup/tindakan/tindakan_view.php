<?php
     ...
     $sql = "select a.*, b.tindakan_split_nominal,b.id_split
               from klinik.klinik_tindakan a
               left join klinik.klinik_tindakan_split b on b.id_tindakan = a.tindakan_id 
               order by a.tindakan_nama, id_split"; 
     $rs = $dtaccess->Execute($sql);
     $dataTable = $dtaccess->FetchAll($rs);
     
     $tot = -1;
     for($i=0,$n=count($dataTable);$i<$n;$i++) {
          if($dataTable[$i]["tindakan_id"]!=$dataTable[$i-1]["tindakan_id"]){
               $tot++;
               $data[$tot] = $dataTable[$i]["tindakan_id"];
          }
          $nama[$dataTable[$i]["tindakan_id"]] = $dataTable[$i]["tindakan_nama"]; 
          $total[$dataTable[$i]["tindakan_id"]] = $dataTable[$i]["tindakan_total"]; 
          $biaya[$dataTable[$i]["tindakan_id"]][$dataTable[$i]["id_split"]] = $dataTable[$i]["tindakan_split_nominal"]; 
     }
