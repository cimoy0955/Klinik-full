<?php
     ...
     if(!$auth->IsAllowed("setup_tindakan_rawat_inap",PRIV_READ)){
          die("access_denied");
          exit(1);
     } else if($auth->IsAllowed("setup_tindakan_rawat_inap",PRIV_READ)===1){
...
/*	if($_POST["btnUpdate"]) {
	...
	$sql = "select * from klinik.klinik_biaya 
          where biaya_jenis like '%T%'
            order by biaya_id, biaya_nama";
    ...
    $counterHeader++;
     ...
     for($i=0,$counter=0,$n=count($dataBiaya);$i<$n;$i++,$counter=0){
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $namaTindakan[$dataBiaya[$i]["biaya_jenis"]];
          $tbContent[$i][$counter][TABLE_ALIGN] = "center";
          $counter++;
