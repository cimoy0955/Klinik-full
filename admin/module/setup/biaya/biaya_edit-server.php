<?php
     ...
     if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
	  ...
	  if($_POST["biaya_jenis"]!='--') $err_code = clearbit($err_code,5);
          ...
          if ($err_code == 0) {
			   ...
			   $dbValue[3] = QuoteValue(DPE_CHAR,$_POST["biaya_total"]);
	...
	$sql = "select * from klinik.klinik_split where split_flag like '".SPLIT_PERAWATAN."' order by split_id";
      ...
      */
     $optJenis[] = $view->RenderOption("--","[-- jenis layanan --]",(!$_POST["biaya_jenis"])?"selected":null,null);
     foreach($biayaStatus as $key => $value){
	  $optJenis[] = $view->RenderOption($key,$value,($_POST["biaya_jenis"]==$key)?"selected":null,null);
     }
...
function setBiayaTotal() {
	var elems = document.querySelectorAll('[id^="txtNom_"]');
	var result=0;
	for(var i=0;i<elems.length;i++){
		if(elems[i].value !== ''){
			result = result + parseInt(elems[i].value);
		}
	}
	...
	//document.getElementById('biaya_total').value = formatCurrency((stripCurrency(document.getElementById('txtNom_1').value) * 1) + (stripCurrency(document.getElementById('txtNom_2').value) * 1));
	 document.getElementById('biaya_total').value = result;
}
</script>

<table width="100%" border="1" cellpadding="1" cellspacing="1">
    <tr class="tableheader">
        <td width="100%">&nbsp;Edit Biaya Tambahan </td>
    </tr>
</table>
...
<tr>
     <td>
     <fieldset>
     <legend><strong>Biaya Tambahan Setup</strong></legend>
     <table width="100%" border="1" cellpadding="1" cellspacing="1">
          <tr>
					...
					<tr>
						<?php for($j=0,$k=count($dataSplit);$j<$k;$j++){ ?>
							<td align="center">
								...
								<?php echo $view->RenderTextBox("txtNom[".$dataSplit[$j]["split_id"]."]","txtNom_".$dataSplit[$j]["split_id"],"10","10",$_POST["txtNom"][$dataSplit[$j]["split_id"]],"curedit", null,false,"onblur=\"setBiayaTotal()\";");?>
							</td>
						<?php } ?>
						<td align="center">
						  ...
						  <?php echo $view->RenderTextBox("biaya_total","biaya_total","10","10",$_POST["biaya_total"],"curedit", null,false,"onfocus=\"setBiayaTotal()\";");?>
						</td>
					</tr>
				</table>
			</td>
