<?php
	...
	elseif($_x_mode=="Edit") $privMode = PRIV_UPDATE;
     ...
     if ($_POST["btnDelete"]) {
          ...
          for($i=0,$n=count($opPaketId);$i<$n;$i++){
               ...
               $sql = "delete from klinik.klinik_biaya_split 
                         where id_biaya = ".QuoteValue(DPE_CHAR,$opPaketId[$i]);
               $dtaccess->Execute($sql,DB_SCHEMA);
          }
	...
	$sql = "select * from klinik.klinik_split where split_flag like '".SPLIT_PERAWATAN."' order by split_id";
     ...
     $dataStatus = $dtaccess->FetchAll($rsStatus);
     for ($i=0; $i < count($dataStatus); $i++) { 
          $optJenis[] = $view->RenderOption($dataStatus[$i]["status_id"],$dataStatus[$i]["status_nama"] ,($_POST["biaya_jenis"] == $dataStatus[$i]["status_id"]) ? "selected" : "",null);
     }
