<?php
	...
	$plx = new InoLiveX("CheckDataCustomerTipe");
     ...
     } elseif($auth->IsAllowed("setup_dokter",PRIV_READ)===1){
	...
	function CheckDataCustomerTipe($custTipeNama)
	{
          ...
          $sql = "SELECT select * from klinik.klinik_biaya
                    WHERE biaya_jenis like '%V%' and upper(a.biaya_nama) = ".QuoteValue(DPE_CHAR,strtoupper($custTipeNama));
	...
	if($_POST["biaya_id"])  $biayaId = & $_POST["biaya_id"];
     ...
     if ($_GET["id"]) {
          ...
          $sql = "select a.* from klinik.klinik_biaya a 
			           	where biaya_jenis like '%V%' and biaya_id = ".QuoteValue(DPE_CHAR,$biayaId);
...
<tr>
     <td>
     <fieldset>
     <legend><strong>Setup Visite</strong></legend>
     <table width="100%" border="1" cellpadding="1" cellspacing="1">
          <tr>
       ...
       <!--   <tr>
          ...
          <td align="right" width="10%">&nbsp;Kelas</td>
              <td width="35%">
                  <select name="biaya_jenis">
    				  <option value="">[ Pilih Kelas Visite ]</option>
    				<?php foreach($namaVisite as $key=>$value) {?>
          ...
          <tr>
               <td colspan="2" align="right">
                    ...
                    <?php echo $view->RenderButton(BTN_BUTTON,"btnBack","btnBack","Kembali","button",false,"onClick=\"document.location.href='dokter_view.php';\"");?>                    
               </td>
          </tr>
     </table>
     </fieldset>
