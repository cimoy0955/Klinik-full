<?php
	...
	$dtaccess = new DataAccess();
     ...
     $adminFindPage = "ref_admin_find.php?";
     ...
     function GetRefraksi($status) {
          ...
          $sql = "select cust_usr_nama,a.reg_id, a.reg_status, a.reg_waktu, a.reg_jadwal, z.fol_lunas, a.reg_jenis_pasien 
				...
				left join (
                    ...
                    where a.reg_status like '".STATUS_REFRAKSI.$status."' and a.reg_tipe_umur = 'D'
          ...
          $tbHeader[0][$counterHeader][TABLE_ISI] = "Jns Pasien";
		...
		$tbHeader[0][$counterHeader][TABLE_ISI] = "Jadwal";
          ...
          for($i=0,$n=count($dataTable),$counter=0;$i<$n;$i++,$counter=0) {
			if($status==0) {
				$tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/bul_arrowgrnlrg.gif" style="cursor:pointer" alt="Proses" title="Proses" border="0" onClick="ProsesRefraksi(\''.$dataTable[$i]["reg_id"].'\')"/>';
			} else {
	...
	if(!$_POST["btnSave"] && !$_POST["btnUpdate"]) {
     ...
     if ($_GET["id"]) {
          ...
          $sql = "select pgw_nama, pgw_id from klinik.klinik_refraksi_admin a
                    join hris.hris_pegawai b on a.id_pgw = b.pgw_id where id_ref = ".QuoteValue(DPE_CHAR,$_POST["ref_id"]);
          $rs = $dtaccess->Execute($sql);
          
          $row=$dtaccess->Fetch($rs);
          $_POST["id_admin"] = $row["pgw_id"];
          $_POST["ref_admin_nama"] = $row["pgw_nama"];
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
	  ...
	  $dbField[39] = "diag_k1_od";
	  $dbField[40] = "diag_k2_od";
	  $dbField[41] = "diag_k1_os";
	  $dbField[42] = "diag_k2_os";
          if($_POST["btnSave"]) $dbField[43] = "ref_when_update";
	  ...
	  $dbValue[39] = QuoteValue(DPE_CHAR,$_POST["diag_k1_od"]);          
          $dbValue[40] = QuoteValue(DPE_CHAR,$_POST["diag_k2_od"]);          
          $dbValue[41] = QuoteValue(DPE_CHAR,$_POST["diag_k1_os"]);          
          $dbValue[42] = QuoteValue(DPE_CHAR,$_POST["diag_k2_os"]);          
          if($_POST["btnSave"]) $dbValue[43] = QuoteValue(DPE_DATE,date("Y-m-d H:i:s"));
         ...
         // insert ke tabel klinik_history_pasien
          ...
          $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["id_reg"]);
          $dbValue[2] = QuoteValue(DPE_CHAR,STATUS_REFRAKSI);
         ...
         // end insert 
          ...
          //}
           if($_POST["id_admin"])    {
               $sqlDelete = "delete from klinik.klinik_refraksi_admin where id_ref = ".QuoteValue(DPE_CHAR,$_POST["ref_id"]);
               $dtaccess->Execute($sqlDelete);
          
                $dbTable = "klinik_refraksi_admin";
                     
                $dbField[0] = "ref_admin_id";   // PK
                $dbField[1] = "id_ref";
                $dbField[2] = "id_pgw";
          ...
          if($_POST["btnSave"]) { 
               // -- insert ke folio jika data ark, streak, diisi ---
	       $lunas = 'n';
	       ...
	       $dbField[10] = "fol_nominal_satuan";
		    ...
		    $folId = $dtaccess->GetTransID();
                    ...
                    $dbValue[5] = QuoteValue(DPE_CHAR,STATUS_REFRAKSI);
				...
				//	$dbTable2 = "klinik_folio_split";
				//
				//	$dbField2[0] = "folsplit_id";   // PK
				//	$dbField2[1] = "id_fol";
				//	$dbField2[2] = "id_split";
				//	$dbField2[3] = "folsplit_nominal";
				//		  
				//	$dbValue2[0] = QuoteValue(DPE_CHAR,$dtaccess->GetTransID());
				//	$dbValue2[1] = QuoteValue(DPE_CHAR,$folId);
				//	$dbValue2[2] = QuoteValue(DPE_CHAR,$dataSplit[$a]["id_split"]);
				//	$dbValue2[3] = QuoteValue(DPE_NUMERIC,$dataSplit[$a]["bea_split_nominal"]);
				//	 
				//	$dbKey2[0] = 0; // -- set key buat clause wherenya , valuenya = index array buat field / value
				//	$dtmodel2 = new DataModel($dbTable,$dbField,$dbValue,$dbKey,DB_SCHEMA_KLINIK);
				//	
				//	$dtmodel2->Insert() or die("insert error"); 
				//	
				//	unset($dtmodel2);
				//	unset($dbField2);
				//	unset($dbValue2);
     ...
     $optionsNext[2] = $view->RenderOption(STATUS_PEMERIKSAAN,"Pemeriksaan",$show);
     $optionsNext[3] = $view->RenderOption(STATUS_LABORATORIUM,"Laboratorium",$show);
...
function viewARK() {
     if (document.getElementById('ck_ark').checked==true) {
	    document.getElementById('tbArk').style.display = "block";
     }else {
	    document.getElementById('tbArk').style.display = "none";   
     }
}
...
function TambahTindakan(){
     ...
     $('#tb_tindakan').createAppend(
          'tr', { class  : 'tablecontent-odd',id:'tr_tindakan_'+akhir+'' },
                ['td', { align: 'left', style: 'color: black;' },
                    [
                         'input', {type:'text', value:'', size:75, maxLength:255, name:'tindakan_nama[]', id:'tindakan_nama_'+akhir},[],
...
<tr>
          ...
          <tr>
               ...
               <td align="left" class="tablecontent-odd" width="80%">
                    <table width="100%" border="0" cellpadding="1" cellspacing="1" id="tb_admin">
                         <tr id="tr_admin_0">
                              <td align="left" class="tablecontent-odd" width="40%">
                                   <?php echo $view->RenderTextBox("ref_admin_nama","ref_admin_nama","30","100",$_POST["ref_admin_nama"],"inputField", "readonly",false);?>       
     ...
     <table border="1" width="100%">
          <tr class="subheader">
               <td width="5%" align="center">Keratometri</td>
     ...
     <fieldset>
     <legend><strong>Tindakan Lain</strong></legend>
     <table width="100%" border="1" cellpadding="4" cellspacing="1" id="tb_tindakan"> 
          ...
          <?php if(!$_POST["tindakan_nama"]) { ?>
               <tr id="tr_tindakan_0">
                    <td align="left" class="tablecontent-odd">
                         <?php echo $view->RenderTextBox("tindakan_nama[0]","tindakan_nama_0","75","255",$_POST["tindakan_nama"][0],"inputField", "readonly",false);?>
                    ...
                    <td align="center" width="30%" class="tablecontent-odd" >
