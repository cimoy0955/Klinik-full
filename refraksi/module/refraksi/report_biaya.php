<?php
     ...
     $sql_where[] = "b.fol_jenis='R'";
     $sql_where[] = "b.fol_nama is not null";
     // === nyari jumlah pemeriksaan refraksi
     $sqlRefraksist = "select fol_nama,count(fol_id) as total from klinik.klinik_refraksi a
                         right join klinik.klinik_folio b on a.id_reg = b.id_reg";
     $sqlRefraksist .= " where ".implode(" and ",$sql_where);
     $sqlRefraksist .= " group by b.fol_nama order by b.fol_nama";
     $rsRefraksist = $dtaccess->Execute($sqlRefraksist);
     //echo $sqlRefraksist;
     // -- end ---
          
     $tableHeader = "Report Biaya Refraksi";
?>
...
<BR>
     ...
     <table width="50%" border="1" cellpadding="1" cellspacing="1">
          <tr>
               <th class="tablesmallheader" width="50%" align="center">Nama Pemeriksaan</th>
               <th width="50%" class="tablesmallheader"  align="center">Jumlah</th>	     
          </tr>
          <?php
          while($dataSusterTotal = $dtaccess->Fetch($rsRefraksist)){
               if($dataSusterTotal["fol_nama"]!=null){
          ?>
          <tr>
               <td class="tablecontent" width="50%" align="leftleft">&nbsp;&nbsp;<?php echo $dataSusterTotal["fol_nama"];?></td>
