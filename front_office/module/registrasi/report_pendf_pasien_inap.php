<?php
     ...
     $thisPage = "report_pendf_pasien_inap.php";
     ...
     $sql_where[] = "c.rawatinap_tanggal_masuk = ".QuoteValue(DPE_DATE,date_db($_POST["tgl_awal"]));

     $sql = "select b.cust_usr_kode, b.cust_usr_nama, b.cust_usr_alamat, b.cust_usr_tanggal_lahir, b.cust_usr_jenis_kelamin, 
               a.reg_jenis_pasien, a.reg_status_pasien, a.reg_keterangan, a.reg_waktu,c.rawatinap_tanggal_masuk,
               c.id_kategori_kamar, d.biaya_nama   
               from klinik.klinik_registrasi a 
               join global.global_customer_user b on a.id_cust_usr = b.cust_usr_id
               join klinik.klinik_rawatinap c on c.id_reg = a.reg_id 
               join klinik.klinik_biaya d on c.id_kategori_kamar = d.biaya_jenis";
     ...
     //*-- config table ---*//
     $tableHeader = "&nbsp;Report Pendaftaran Pasien Rawat Inap Harian";
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "Kelas Kamar";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "10%";     
     $counterHeader++;
	...
	if($_POST["btnExcel"]){
          ...
          header('Content-Disposition: attachment; filename=report_pendf_pasien_inap_'.$_POST["tgl_awal"].'.xls');
     }
