...
//     }
	...
	if ($_POST["btnUpdate"]) { 
		...
		/* update tgl 22 Feb 14
		 * update insert ke tabel klinik_folio
		 * untuk menambahkan tagihan cetak kartu pasien
		 */
		
		$sql_reg = "select reg_id,max(reg_tanggal) as max_tanggal,max(reg_waktu) as max_waktu
				 from klinik.klinik_registrasi
				 where id_cust_usr=".QuoteValue(DPE_CHAR,$_POST["cust_usr_id"])."
				 group by reg_id, reg_tanggal, reg_waktu
				 order by reg_tanggal desc, reg_waktu desc limit 1";
		$rs_reg = $dtaccess->Execute($sql_reg);
		$data_reg = $dtaccess->Fetch($rs_reg);
		
		$sql = "select * from klinik.klinik_biaya where biaya_kode = ".QuoteValue(DPE_CHAR,"RJ1-01");
		 ...
		 $dbValue[1] = QuoteValue(DPE_CHAR,$data_reg["reg_id"]);
