<?php
    ...
    $plx = new InoLiveX("CheckKode,GetReg,CekUmur");     
  ...
  // ADD / EDIT DATA
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {          
	  ...
	  /*
	  if($_POST["cust_usr_telp"]||$_POST["cust_usr_hp"]) $err_code = clearbit($err_code,4);
	  ...
	  */
	  if($err_code == 0){
	    ...
	    if($_POST["cust_usr_jenis"]==PASIEN_DINASLUAR){
	      $dbField[16] = "reg_dinasluar_tanggal";
	      $dbField[17] = "reg_dinasluar_kota";
	    }
	    	    
	    // untuk pasien dibawah 15th statusnya A,masuk ke antrian polianak
	    if(HitungUmur(date_db($_POST["cust_usr_tanggal_lahir"]))>15){
		$regStatusUmur = "D";
		$regStatus = STATUS_REFRAKSI;
	    ...
	    if($_POST["cust_usr_jenis"]==PASIEN_DINASLUAR){
		$dbValue[16] = QuoteValue(DPE_DATE,date_db($_POST["reg_dinasluar_tanggal"]));
		$dbValue[17] = QuoteValue(DPE_CHAR,$_POST["reg_dinasluar_kota"]);
	    }
