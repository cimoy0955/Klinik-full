<?php
    ...
    $plx = new InoLiveX("CheckKode,GetReg,CekUmur,get_rujukan_rs,get_rujukan_dokter,getOptDomisili");     
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {          
	  ...
	  */
	  ...
	  if($err_code == 0){
	    ...
	    $dbField[26] = "cust_usr_kota_domisili";
	    $dbField[27] = "cust_usr_propinsi_domisili";
	    ...
	    $dbValue[26] = QuoteValue(DPE_NUMERIC,$_POST["cust_usr_propinsi_domisili"]);
	    $dbValue[27] = QuoteValue(DPE_NUMERIC,$_POST["cust_usr_kota_domisili"]);
...
*/
 ...
 //    $optStr = "";
 // 	$optStr = get_rujukan_rs();
	// $optStr = json_decode($optStr);
	// // print_r($optStr);
	$optRujukanID[] = $view->RenderOption("--","--","selected");
	// for($i=0;$i<count($optStr);$i++){
	// 	$optRujukanID[] = $view->RenderOption($optStr[$i]->id,$optStr[$i]->nama,($_POST["id_rujukan_rs"]==$optStr[$i]->id)?"selected":"",null);
	// }
     
 //    $optStr2 = "";
 //    $optStr2 = get_rujukan_dokter();
	// $optStr2 = json_decode($optStr2);
	// // print_r($optStr);
	$optRujukanDokterID[] = $view->RenderOption("--","--","selected");
	// for($i=0;$i<count($optStr2);$i++){
	// 	$optRujukanDokterID[] = $view->RenderOption($optStr2[$i]->id,$optStr2[$i]->nama,($_POST["id_rujukan_dokter"]==$optStr2[$i]->id)?"selected":"",null);
	// }
...
<table width="100%" border="1" cellpadding="4" cellspacing="1" id="tabel_reg">
  ...
  <tr>
	...
	<tr>
		<td width= "20%" class="tablecontent">Alamat<?php if(readbit($err_code,3) || readbit($err_code,15)) {?>&nbsp;<font color="red">(*)</font><?}?><br /><span class="hint-label">(sesuai KTP/tanda pengenal)</span></td>
		<td class="tablecontent-odd">
