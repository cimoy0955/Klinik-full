<?php
	...
	$auth = new CAuth();
     ...
     if ($_POST["btnDelete"]) {
	  $regId = & $_POST["cbDelete"];
	  for($i=0,$n=count($regId);$i<$n;$i++){
	       $sql = "update klinik.klinik_registrasi set reg_status = ".QuoteValue(DPE_CHAR,STATUS_SELESAI."0")." where reg_id = ".QuoteValue(DPE_CHAR,$regId[$i]);
	       $dtaccess->Execute($sql);
	  }
	  header("location:edit_pasien_view.php");
	  exit();    
