<?php
	...
	$sql = "select a.reg_status, a.reg_id, b.cust_usr_kode, cust_usr_nama, id_cust_usr 
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){ 
          ...
          if($auth->IsAllowed("setup_role",PRIV_DELETE)) {
	       if($dataTable[$i]["reg_status"]{0}=="R" || $dataTable[$i]["reg_status"]{0}=="A" || $dataTable[$i]["reg_status"]{0}=="I") $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;"; 
               else $tbContent[$i][$counter][TABLE_ISI] = '<input type="checkbox" name="cbDelete[]" value="'.$dataTable[$i]["reg_id"].'">';
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $rawatStatus[$dataTable[$i]["reg_status"]{0}];//."&nbsp;".$dataTable[$i]["reg_status"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
          $counter++;
     }
     ...
     if($auth->IsAllowed("setup_role",PRIV_DELETE) && $auth->IsAllowed("setup_role",PRIV_CREATE)) {
          $tbBottom[0][0][TABLE_ISI] = '&nbsp;&nbsp;<input type="submit" name="btnDelete" value="Selesai" class="button">';
     };
