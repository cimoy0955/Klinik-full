...
function GetData($in_nama=null){
	...
	if($_POST["btnSearch"]){
	if($in_nama) $sql_where = "and UPPER(biaya_nama) like '%".strtoupper($in_name)."%'"; 
	 }
/* 
  ...
  $tbHeader[0][$counter][TABLE_ISI] = "Biaya";
    ...
    $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;Rp. ".currency_format($dataTable[$i]["biaya_total"]);
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img style="cursor:pointer;" src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.addslashes(htmlentities($dataTable[$i]["biaya_nama"])).'\',\''.$dataTable[$i]["biaya_id"].'\',\''.$dataTable[$i]["biaya_jenis"].'\',\''.currency_format($dataTable[$i]["biaya_total"]).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
