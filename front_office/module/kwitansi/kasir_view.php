<?php
     ...
     function GetFolio() {
          global $dtaccess, $view, $tableRefraksi, $thisPage, $APLICATION_ROOT,$rawatStatus,$namaFolio; 
               
          $sql = "select a.id_reg, a.id_cust_usr, b.cust_usr_nama,a.fol_jenis,a.fol_waktu,id_biaya,a.fol_lunas, a.fol_id
                  ...
                  where a.fol_lunas = 'n' 
                   and fol_jenis = ".QuoteValue(DPE_CHAR,KARCIS_LOKET)."
		...
		for($i=0,$n=count($dataTable);$i<$n;$i++) {
			...
			if($dataTable[$i]["id_reg"]!=$dataTable[$i-1]["id_reg"] || $dataTable[$i]["fol_jenis"]!=$dataTable[$i-1]["fol_jenis"]) {
				...
				$idfol[$dataTable[$i]["id_reg"]] = $dataTable[$i]["fol_id"];
			}
		}
          ...
          for($i=0,$nomor=1,$n=count($data),$counter=0;$i<$n;$i++,$counter=0) {
			   ...
			   $editPage = $thisPage."?jenis=".$fol[$data[$i]][$i]."&id_reg=".$reg[$data[$i]]."&waktu=".$waktu[$data[$i]]."&fol_id=".$idfol[$data[$i]];
				
        if($fol[$data[$i]][$i]==STATUS_PEMERIKSAAN)
				$editPage .= "&biaya=".$biaya[$data[$i]][$i];
				
    			$tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'"><img hspace="2" width="15" height="15" src="'.$APLICATION_ROOT.'images/b_select.png" alt="Proses" title="Proses" border="0"/></a>';               
    			...
    			$tbContent[$i][$counter][TABLE_ISI] = $namaFolio[$fol[$data[$i]][$i]];
    			$tbContent[$i][$counter][TABLE_ALIGN] = "left";
    			$counter++; 
            }
  ...
  //             $dbField[11] = "id_biaya_tambahan";
               ...
               if(!$_POST["id_biaya"]) $_POST["id_biaya"] =  $_POST["biaya_id_tambahan"] ;
  ...
  //echo $sql;
	...
	//echo $sql;
     ...
     if($_POST["btnDelete"]){
      ...
      $delPage = $thisPage."?jenis=".$_POST["fol_jenis"]."&id_reg=".$_POST["id_reg"]."&waktu=".$_POST["waktu"]."&fol_id=".$_POST["fol_id"];
      header("location:".$delPage);
      //exit();
   }
...
//	$isprint = "n";
		...
		//echo $sql;
    $dtaccess->Execute($sql);
		$sql = "update klinik.klinik_registrasi set reg_status_ugd = 'n' where reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
		$dtaccess->Execute($sql);
...
{
    if(!_wnd_new) {
			_wnd_new = window.open(url,judul,'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=560,height=600,left=100,top=100');
	} else {
		if (_wnd_new.closed) {
...
<?php if($isprint=="y" && $_POST["id_reg"] && $_POST["fol_jenis"]){ ?>
     if(confirm('Cetak Invoice?')) BukaWindow('kasir_cetak.php?jenis=<?php echo $_POST["fol_jenis"];?>&id_reg=<?php echo $_POST["id_reg"];?>','Invoice');
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
		<tr>
			<td align="left">
				...
				<?php echo $view->RenderButton(BTN_SUBMIT,($_x_mode == "Edit") ? "btnUpdate" : "btnSave","btnSave","Bayar","button",false,'onClick="BukaWindow(\'kasir_cetak.php?jenis='.$_POST["fol_jenis"].'&id_reg='.$_POST["id_reg"].'\',\'Cetak Invoice\')"',null);?> <!--,'onClick="CheckSimpan()"');?>-->
