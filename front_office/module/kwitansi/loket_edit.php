<?php
     ...
     $_x_mode = "New";
     $thisPage = "loket_edit.php";
     $viewPage = "report_loket.php";
     $findPage = "item_find2.php?";
     ...
     $sql_where[] = "fol_waktu is not null and fol_lunas = ".QuoteValue(DPE_CHAR,"y"); 
     
     if($_POST["tgl_awal"]) $sql_where[] = "CAST(fol_waktu as DATE) >= ".QuoteValue(DPE_DATE,date_db($_POST["tgl_awal"]));
     if($_POST["tgl_akhir"]) $sql_where[] = "CAST(fol_waktu as DATE) <= ".QuoteValue(DPE_DATE,date_db($_POST["tgl_akhir"]));
	...
	$sql_where = implode(" and ",$sql_where);
     ...
     function GetFolio() {
          ...
          $sql = "select a.id_reg, a.id_cust_usr, b.cust_usr_nama,a.fol_jenis,a.fol_waktu,id_biaya,a.fol_lunas,b.cust_usr_jenis, z.reg_status
        ...
        if($_POST["btnSaveTambah"]){
            ...
            $tanggal = date("Y-m-d H:i:s");
               ...
               $dbValue[8] = QuoteValue(DPE_CHAR,'y');
               $dbValue[9] = QuoteValue(DPE_NUMERIC,StripCurrency($_POST["txtHargaTotal"]));
               $dbValue[10] = QuoteValue(DPE_DATE,$tanggal);
     ...
     $sql = "select * from klinik.klinik_biaya_split where id_biaya = ".QuoteValue(DPE_CHAR,$_POST["biaya_id"]);
				//	echo $sql;
...
<tr>
     ...
     <table width="80%" border="1" cellpadding="4" cellspacing="1">
		...
		<input type="submit" name="btnDelete" value="X" class="button">
			</td>
