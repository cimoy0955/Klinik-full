...
function GetData($in_nama,$in_kode){
	...
	// --- cari data krsnya ---
	$sql = "select obat_id, obat_nama, obat_kode,obat_stok, id_petunjuk, petunjuk_nama from apotik.apotik_obat_master a
          left join apotik.apotik_obat_petunjuk b on b.petunjuk_id = a.id_petunjuk";
	$sql .= " where ".$sql_where;			
	$sql .= " order by a.obat_nama";
	...
	$tbHeader[0][$counter][TABLE_ISI] = "Dosis";
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$dataTable[$i]["obat_stok"];
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
		
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["obat_id"].'\',\''.$dataTable[$i]["petunjuk_nama"].'\',\''.addslashes(htmlentities($dataTable[$i]["obat_nama"])).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,petunjuk,nama) {
		
	self.parent.document.getElementById('obat_nama_<?php echo $_GET["el"];?>').value = nama;
	self.parent.document.getElementById('petunjuk_nama_<?php echo $_GET["el"];?>').value = petunjuk;
	self.parent.document.getElementById('obat_id_<?php echo $_GET["el"];?>').value = id;
