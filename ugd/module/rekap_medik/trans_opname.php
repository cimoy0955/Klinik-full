<?php
     ...
     if(!$auth->IsAllowed("perawatan",PRIV_READ)){
          die("access_denied");
          exit(1);
          
     } elseif($auth->IsAllowed("perawatan",PRIV_READ)===1){
  ...
  $sql = "select * from logistik.pos_saldo_ugd";
  ...
  $dataItem = $dtaccess->FetchAll($rs_edit); 
     ...
     {
     $dbTable = "logistik.pos_saldo_ugd";
     
     $dbField[0] = "saldo_id";   // PK
     $dbField[1] = "saldo_nama";
     $dbField[2] = "saldo_stok_awal";
     
     
     $saldoId = $dtaccess->GetTransID();   
     $dbValue[0] = QuoteValue(DPE_CHAR,$saldoId);
     $dbValue[1] = QuoteValue(DPE_CHAR,$dataItem[$i]["saldo_nama"]);
...
<td width="70%">
    <fieldset>
          ...
          <tr  class="<?php if($i%2==0) echo 'tablecontent-odd'; else echo 'tablecontent'; ?>">
               <td align="center">
                    <?php echo $view->RenderLabel("saldo_nama","saldo_nama",$dataItem[$i]["saldo_nama"], null,false);?>
               </td>
               <td>
                    <?php echo $view->RenderTextBox("stokHand$i","stokHand$i","20","40",$dataItem[$i]["saldo_stok_awal"],"inputField", null,false);?>
               </td>
