<?php
	...
	$susterPage = "rawat_suster_find.php?";
     ...
     function GetPerawatan($status) {
          global $dtaccess, $view, $tableRefraksi, $thisPage, $APLICATION_ROOT,$userId,$poliId; 
           
               
          $sql = "select  distinct(z.fol_lunas) ,cust_usr_nama,a.reg_id, a.reg_status, a.reg_waktu, a.reg_jadwal,a.reg_tanggal,c.poli_nama 
                    from klinik.klinik_registrasi a
                    left join global.global_customer_user b on a.id_cust_usr = b.cust_usr_id
				            left join klinik.klinik_folio z on a.reg_id = z.id_reg
				            left join global.global_auth_poli c on a.id_poli = c.poli_id 
                    where a.reg_status like '".STATUS_PEMERIKSAAN.$status."' and a.reg_ugd = 'n' 
                    and a.id_poli = ".QuoteValue(DPE_NUMERIC,$poliId)."
...
//	echo $_POST["obat_id"]."-".$_POST["obat_id"];
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
          ...
          $dbValue[66] = QuoteValue(DPE_CHAR,'M'); 
          $dbValue[67] = QuoteValue(DPE_CHAR,'RAWAT JALAN'); 
		...
		$dtaccess->Execute($sql); 
          ...
          for($i=0,$n=count($_POST["obat_id"]);$i<$n;$i++) {
               $dbValue[0] = QuoteValue(DPE_CHARKEY,$dtaccess->GetTransID());
               $dbValue[1] = QuoteValue(DPE_CHARKEY,$_POST["rawat_id"]);
               $dbValue[2] = QuoteValue(DPE_CHARKEY,$_POST["obat_id"][$i]);
               $dbValue[3] = QuoteValue(DPE_NUMERIC,$i);
               ...
               if($_POST["obat_id"][$i]) $dtmodel->Insert() or die("insert  error");
	...
	$sketsaName = ($_POST["rawat_mata_sketsa"]) ? $lokasi."/".$_POST["rawat_mata_sketsa"] : $lokasi."/default.jpg";
     ...
     $sql = "select obat_id, obat_nama from apotik.apotik_obat_master where id_menu = ".QuoteValue(DPE_NUMERIC,3);
     ...
     for($i=0,$n=count($dataAnestesisObat);$i<$n;$i++) {
          $show = ($_POST["rawat_anestesis_obat"]==$dataAnestesisObat[$i]["obat_id"]) ? "selected":"";
          $optAnestesisObat[$i+1] = $view->RenderOption($dataAnestesisObat[$i]["obat_id"],$dataAnestesisObat[$i]["obat_nama"],$show); 
     }

     $optTerapiObat[0] = $view->RenderOption("","[Pilih Obat Terapi]",$show); 
     for($i=0,$n=count($dataAnestesisObat);$i<$n;$i++) {
          $show = ($_POST["rawat_anestesis_obat"]==$dataAnestesisObat[$i]["obat_id"]) ? "selected":"";
          $optTerapiObat[$i+1] = $view->RenderOption($dataAnestesisObat[$i]["obat_id"],$dataAnestesisObat[$i]["obat_nama"],$show); 
     }
?>
...
function Tambah(){
     ...
     $('#tb_terapi').createAppend(
          'tr', { class  : 'tablecontent-odd',id:'tr_terapi_'+akhir+'' },
                ['td', { align: 'left', style: 'color: black;' },
                    [
                         'input', {type:'text', value:'', size:20, maxLength:100, name:'obat_nama[]', id:'obat_nama_'+akhir},[],
                         ...
                         'input', {type:'hidden', value:'', name:'obat_id[]', id:'obat_id_'+akhir+''}
                    ],
               'td', { align: 'center', style: 'color: black;' },
                    [
                         'span', {id:'id_petunjuk_'+akhir+''}
                    ],
               'td', { align: 'center', style: 'color: black;' },
                       [
     ...
     document.getElementById('obat_nama_'+akhir).readOnly = true;
...
function Delete(akhir){
     document.getElementById('hid_id_del').value += document.getElementById('obat_id_'+akhir).value;
...
function Delete1(akhir){
     document.getElementById('hid_id_del').value += document.getElementById('tindakan_id_'+akhir).value;
...
<tr>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1" id="tb_terapi"> 
          ...
          <?php if(!$_POST["obat_nama"]) { ?>
               <tr  class="tablecontent-odd" id="tr_terapi_0">
                    <td align="left" class="tablecontent-odd">
                         <?php echo $view->RenderTextBox("obat_nama[0]","obat_nama_0","20","100",$_POST["obat_nama"][0],"inputField", "readonly",false);?>
                         ...
                         <input type="hidden" name="obat_id[0]" id="obat_id_0" value="<?php echo $_POST["obat_id"][0]?>" />                    
                    </td>
                    <td align="center" class="tablecontent-odd"><span id="id_petunjuk_0"></span></td>
          ...
          <?php } else { ?>
               <?php for($i=0,$n=count($_POST["obat_id"]);$i<$n;$i++) { ?>
                    <tr id="tr_terapi_<?php echo $i;?>">
                         <td align="left" class="tablecontent-odd" width="70%">
                              <?php echo $view->RenderTextBox("obat_nama[]","obat_nama_".$i,"30","100",$_POST["obat_nama"][$i],"inputField", "readonly",false);?>
                         ...
                         <td align="center" class="tablecontent-odd"><span id="id_petunjuk_<?php echo $i;?>"><?php echo GetDosis($_POST["id_petunjuk"][$i]);?></span></td>
                         <td align="left" class="tablecontent-odd" width="30%">
                              <?php if($i==0) { ?>
                              <input class="button" name="btnAdd" id="btnAdd" type="button" value="Tambah" onClick="Tambah();">
                              <?php } else { ?>
	...
	<fieldset>
     <legend><strong>Jenis Tindakan</strong></legend>
          ...
          <?php if(!$_POST["tindakan_nama"]) { ?>
               <tr  class="tablecontent-odd" id="tr_tindakan_0">
                    <td align="left" class="tablecontent-odd">
                         ...
                         <input type="hidden" name="tindakan_id[0]" id="tindakan_id_0" value="<?php echo $_POST["tindakan_id"][0]?>" />                    
                    </td>
                    <td align="center" class="tablecontent-odd"><span id="id_petunjuk_0"></span></td>
                    <td align="center" class="tablecontent-odd">
                         <input class="button" name="btnAdd" id="btnAdd" type="button" value="Tambah" onClick="Tambah();">
