<?php
	...
	$tree = new CTree("global.global_customer","cust_id",TREE_LENGTH);
     ...
     if(!$auth->IsAllowed("edit_pemeriksaan_ugd",PRIV_CREATE)){
          die("access_denied");
          exit(1);
     } else if($auth->IsAllowed("edit_pemeriksaan_ugd",PRIV_CREATE)===1){
          echo"<script>window.parent.document.location.href='".$APLICATION_ROOT."login.php?msg=Login First'</script>";
          exit(1);
     }

     $_x_mode = "New";
     $thisPage = "perawatan_view.php";
     $editPage = "perawatan.php?";
	...
	if($_POST["cust_usr_id"]) {
     ...
     if($dataPasien) {
          ...
          $sql = "select a.rawat_id,  rawat_tanggal from klinik.klinik_perawatan a where a.id_cust_usr = ".QuoteValue(DPE_NUMERIC,$_POST["cust_usr_id"])." 
                    order by a.rawat_tanggal desc";
          ...
          $isAllowedUpdate = $auth->IsAllowed("perawatan",PRIV_UPDATE);
          ...
          for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
               ...
               if($isAllowedUpdate) {
                    $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataTable[$i]["rawat_id"]).'"><img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/b_edit.png" alt="Edit" title="Edit" border="0"></a>';               
...
<table width="100%" border="0" cellpadding="4" cellspacing="1">
	<tr>
		<td align="left" colspan=2 class="tableheader">Data Pemeriksaan</td>
	</tr>
</table> 
