...
function GetData($in_nama,$in_kode){
	...
	$sql_where[] = "1=1"; 
	if($in_nama) $sql_where[] = "UPPER(ina_nama) like ".QuoteValue(DPE_CHAR,strtoupper("%".$in_nama."%"));
	if($in_kode) $sql_where[] = "UPPER(ina_kode) like ".QuoteValue(DPE_CHAR,strtoupper("%".$in_kode."%"));
	$sql_where = implode(" and ",$sql_where);

	// --- cari data krsnya ---
	$sql = "select distinct(ina_id), ina_nama, ina_kode from klinik.klinik_ina a";
	$sql .= " where ".$sql_where;			
	$sql .= " order by a.ina_kode";
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$dataTable[$i]["ina_nama"];
		$tbContent[$i][$counter][TABLE_ALIGN] = "left";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
		
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["ina_id"].'\',\''.$dataTable[$i]["ina_kode"].'\',\''.addslashes(htmlentities($dataTable[$i]["ina_nama"])).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,kode,nama) {
	...
	self.parent.document.getElementById('rawat_ina_<?php echo $_GET["tipe"];?>_kode_<?php echo $_GET["el"];?>').value = kode;
	self.parent.document.getElementById('rawat_ina_<?php echo $_GET["tipe"];?>_nama_<?php echo $_GET["el"];?>').value = nama;
	self.parent.document.getElementById('rawat_ina_<?php echo $_GET["tipe"];?>_id_<?php echo $_GET["el"];?>').value = id;
...
<tr>
	<td>
		<table cellpadding="1" cellspacing="1" border="1" align="center" width="100%">
			...
			<tr>
				<td align="right" class="tablecontent" width="30%">INA DRG</td>
