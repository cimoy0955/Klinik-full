	...
	if($_GET["id_reg"]) {
     ...
     if($_POST["btnSave"] || $_POST["btnUpdate"]) {
		...
		$dtaccess->Execute($sql); 
          ...
          // --- insrt suster ---
          $sql = "delete from klinik.klinik_perawatan_terapi_suster where id_op = ".QuoteValue(DPE_CHAR,$_POST["asisten_op_id"]);
          $dtaccess->Execute($sql);
          
          if($_POST["id_asisten_suster"]) {
               foreach($_POST["id_asisten_suster_terapi"] as $key => $value){
                    if($value) {
                         $dbTable = "klinik_operasi_asisten_suster";
                         ...
                         $dbValue[0] = QuoteValue(DPE_CHAR,$dtaccess->GetTransID());
                         $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["asisten_op_id"]);
                         $dbValue[2] = QuoteValue(DPE_NUMERICKEY,$value);
...
function AsistenSusterTambah(){
     ...
     $('#tb_asisten_suster').createAppend(
          'tr', { class  : 'tablecontent-odd',id:'tr_asisten_suster__'+akhir+'' },
                ['td', { align: 'left', style: 'color: black;' },
                    [
                         'input', {type:'text', value:'', size:30, maxLength:100, name:'op_asisten_suster_nama[]', id:'op_asisten_suster_nama'+akhir},[],
                         ...
                         'input', {type:'hidden', value:'', name:'id_asisten_suster[]', id:'id_asisten_suster'+akhir+''}
                    ],
               'td', { align: 'left', style: 'color: black;' },
                       [
                            'input', {type:'button', class:'button', value:'Hapus', name:'btnDel['+akhir+']', id:'btnDel_'+akhir}
     ...
     document.getElementById('op_asisten_suster_nama'+akhir).readOnly = true;
          ...
          <tr>
              ...
              <!-- <td align="left" width="20%" class="tablecontent">Asisten Perawat</td>
               ...
               <td align="left" class="tablecontent-odd" width="80%">
				<table width="100%" border="0" cellpadding="1" cellspacing="1" id="tb_asisten_suster">
                         <?php if(!$_POST["op_asisten_suster_nama"]) { ?>
					<tr id="tr_asisten_suster__0">
						<td align="left" class="tablecontent-odd" width="70%">
							<?php echo $view->RenderTextBox("op_asisten_suster_nama[]","op_asisten_suster_nama_0","30","100",$_POST["op_asisten_suster_nama"][0],"inputField", "readonly",false);?>
							<a href="<?php echo $asistenSusterPage;?>&el=0&TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Cari Asisten Suster"><img src="<?php echo($APLICATION_ROOT);?>images/bd_insrow.png" border="0" align="middle" width="18" height="20" style="cursor:pointer" title="Cari Asisten Suster" alt="Cari Asisten Suster" /></a>
							<input type="hidden" id="id_asisten_suster_0" name="id_asisten_suster[]" value="<?php echo $_POST["id_asisten_suster"];?>"/>
			               </td>
						<td align="left" class="tablecontent-odd" width="30%">
                    ...
                    <?php } else  { ?>
                         <?php for($i=0,$n=count($_POST["id_asisten_suster"]);$i<$n;$i++) { ?>
                              <tr id="tr_asisten_suster_<?php echo $i;?>">
                                   <td align="left" class="tablecontent-odd" width="70%">
                                        <?php echo $view->RenderTextBox("op_asisten_suster_nama[]","op_asisten_suster_nama".$i,"30","100",$_POST["op_asisten_suster_nama"][$i],"inputField", "readonly",false);?>
                                        <?php //if($edit) {?>
                                             <a href="<?php echo $asistenSusterPage;?>&el=<?php echo $i;?>&TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Cari Suster"><img src="<?php echo($APLICATION_ROOT);?>images/bd_insrow.png" border="0" align="middle" width="18" height="20" style="cursor:pointer" title="Cari Suster" alt="Cari Suster" /></a>
                                        <?php //}?>
                                        <input type="hidden" id="id_asisten_suster_<?php echo $i;?>" name="id_asisten_suster_[]" value="<?php echo $_POST["id_asisten_suster"][$i];?>"/>
                                   </td>
                                   <td align="left" class="tablecontent-odd" width="30%">
                                        <?php// if($edit) {?>
                                             <?php if($i==0) { ?>
                         ...
                         <?php } ?>
                    <?php } ?>
                    <?php echo $view->RenderHidden("hid_asisten_suster_del","hid_asisten_suster_del",'');?>
				</table>
               </td>
          </tr>
          <tr>
