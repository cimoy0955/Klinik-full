<?php
     ...
     if(!$auth->IsAllowed("report_pasien_ugd",PRIV_READ)){
          die("access_denied");
          exit(1);
          
     } elseif($auth->IsAllowed("report_pasien_ugd",PRIV_READ)===1){
...
<table align="center" border=0 cellpadding=2 cellspacing=1 width="100%" class="tblForm" id="tblSearching">
     <tr>
          ...
          <td width="25%" class="tablecontent-odd">
			<select name="cust_usr_kode" id="cust_usr_kode" onKeyDown="return tabOnEnter(this, event);">
                    <option value="" >[ Pilih Jenis Pasien ]</option>
                    <?php foreach($bayarPasien as $key => $value) { ?>
                         <option value="<?php echo $key;?>" <?php if($key==$_POST["cust_usr_kode"]) echo "selected"; ?>><?php echo $value;?></option>
                    <?php } ?>
			</select>
          </td>
