<?php
     ...
     $sqlPasien = "select c.icd_id,c.icd_nomor,c.icd_nama,count(a.id_reg) as jumlah_perawatan
                    ...
                    left join klinik.klinik_icd c on b.id_icd = c.icd_id
                    where c.icd_jenis = '1' ";
     ...
     $sqlPasien = "select c.ina_id,c.ina_kode,c.ina_nama,count(a.id_reg) as jumlah_perawatan
                    from klinik.klinik_perawatan a
                    left join klinik.klinik_perawatan_ina b on a.rawat_id = b.id_rawat
                    left join klinik.klinik_ina c on b.id_ina = c.ina_id
                    where c.ina_jenis = '1' ";
     $sql_where_ina = $sql_where;
     $sqlPasien = $sqlPasien." and ".implode(" and ",$sql_where_ina);
     $sqlPasien .=" group by c.ina_id,c.ina_kode,c.ina_nama ";
     $rsPasien = $dtaccess->Execute($sqlPasien);
    ...
    */ // -- end ---
     ...
     $tbContent2[$i][$counterContent][TABLE_ISI] = $dataPasienINA[$i]["ina_kode"];
     $tbContent2[$i][$counterContent][TABLE_WIDTH] = "3%";
     $tbContent2[$i][$counterContent][TABLE_ALIGN] = "center";
     $counterContent++;
      
     $tbContent2[$i][$counterContent][TABLE_ISI] = "&nbsp;&nbsp;&nbsp;".$dataPasienINA[$i]["ina_nama"];
