<?php
	...
	if($_POST["id_cust_usr"]) {
		$sql = "select cust_usr_nama,cust_usr_kode, b.cust_usr_jenis_kelamin, ((current_date - cust_usr_tanggal_lahir)/365) as umur 
	     ...
	     $sql = "select * from klinik.klinik_perawatan where id_cust_usr = ".QuoteValue(DPE_CHAR,$_POST["id_cust_usr"])." and id_reg <> ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
...
<?php echo $view->RenderBody("inosoft.css",false,null,false,"inventori_prn.css"); ?>
<link rel="stylesheet" href="<?php echo $APLICATION_ROOT; ?>/library/css/jquery-ui.css">
<script src="<?php echo $APLICATION_ROOT;?>library/script/jquery-ui/jquery-1.9.1.js"></script>
<script src="<?php echo $APLICATION_ROOT;?>library/script/jquery-ui/jquery-ui.js"></script>
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" id="noborder">
	<tr>
		<td align="left" colspan=2 class="tableheader" class="noborder">Rekap Medik</td>
	</tr> 
</table> 
...
<?php for($i=0,$n=count($dataPemeriksaan);$i<$n;$i++) { ?> 
     <h3><?php echo format_date(substr($dataPemeriksaan[$i]["rawat_waktu"],0,10));?></h3>
     <div>Petugas
