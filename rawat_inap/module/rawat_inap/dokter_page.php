...
*/
    ...
    function GetDosis($akhir,$id=null) {
          global $dtaccess, $view;
          
          $sql = "select petunjuk_id, petunjuk_nama from logistik.logistik_item";
          $dataTable = $dtaccess->FetchAll($sql);

          $optDosis[0] = $view->RenderOption("","[Pilih Dosis Obat]",$show); 
          for($i=0,$n=count($dataTable);$i<$n;$i++) {
               $show = ($id==$dataTable[$i]["petunjuk_id"]) ? "selected":"";
               $optDosis[$i+1] = $view->RenderOption($dataTable[$i]["petunjuk_id"],$dataTable[$i]["petunjuk_nama"],$show); 
          }
          
          return $view->RenderComboBox("id_petunjuk[]","id_petunjuk_".$akhir,$optDosis,null,null,null);
     }
	...
	if(!$_POST["btnSaveRawat"] && !$_POST["btnUpdateRawat"]) {
     	...
     	// --- buat cari pemeriksaan yg tugas hari ini --- 
          $sql = "select distinct pgw_nama, pgw_id from klinik.klinik_perawatan_dokter a
                    join hris.hris_pegawai b on a.id_pgw = b.pgw_id 
                    join klinik.klinik_perawatan c on c.rawat_id = a.id_rawat
                    where c.rawat_tanggal = ".QuoteValue(DPE_DATE,date('Y-m-d')); 
          $rs = $dtaccess->Execute($sql);
          ...
          $i=0;     
          while($row=$dtaccess->Fetch($rs)) {
               if(!$_POST["id_dokter"][$i]) $_POST["id_dokter"][$i] = $row["pgw_id"];
               if(!$_POST["rawat_dokter_nama"][$i]) $_POST["rawat_dokter_nama"][$i] = $row["pgw_nama"];
               $i++;
          }
     ...
     if ($_GET["id"]) {
          ...
          $pemeriksaanLink = "perawatan_perawat_visite.php?id_cust_usr=".$enc->Encode($dataPasien2["id_cust_usr"])."&id_reg=".$enc->Encode($_GET["id_reg"]); 
	...
	if($_GET["id_reg"] && $_GET["id_rawatinap"]) {
		$sql = "select cust_usr_nama,cust_usr_kode, b.cust_usr_jenis_kelamin, a.reg_jenis_pasien, 
            ...
            d.id_bed, d.id_kamar  ,d.id_kategori_kamar
            from klinik.klinik_registrasi a
            left join klinik.klinik_refraksi c on a.reg_id = c.id_reg 
            left join global.global_customer_user b on a.id_cust_usr = b.cust_usr_id
            left join klinik.klinik_rawatinap d on d.id_reg = a.reg_id  
            where a.reg_id = ".QuoteValue(DPE_CHAR,$_GET["id_reg"]);
          $dataPasien2= $dtaccess->Fetch($sql);
          //echo $sql;
        	$cug = $_GET["id_reg"]; 
        	$_POST["id_rawatinap"] = $_GET["id_rawatinap"];
        	$_POST["id_cust_usr"] = $dataPasien2["id_cust_usr"];
          $_POST["reg_jenis_pasien"] = $dataPasien2["reg_jenis_pasien"];  
        	$_POST["rawat_keluhan"] = $dataPasien2["ref_keluhan"];
        	$_POST["bed_id"] = $dataPasien2["id_bed"];
        	$_POST["id_kamar"] = $dataPasien2["id_kamar"];        	
          $_POST["id_kategori_kamar"] = $dataPasien2["id_kategori_kamar"]; 
          ...
          $_POST["kontrol_id"] = $data_kontrol["kontrol_id"];
                   ...
                   $pemeriksaanLink = "perawatan_perawat_visite.php?id_cust_usr=".$enc->Encode($dataPasien2["id_cust_usr"])."&id_reg=".$enc->Encode($_GET["id_reg"]);
          $diagLink = "perawatan_diag.php?id_cust_usr=".$enc->Encode($dataPasien2["id_cust_usr"])."&id_reg=".$enc->Encode($_GET["id_reg"]);
		...
		//-- ina klaim--
          $sql = "select ina_nama, ina_kode, ina_id 
				from klinik.klinik_registrasi_ina a
                    join klinik.klinik_ina b on a.id_ina = b.ina_id
				where id_reg = ".QuoteValue(DPE_CHAR,$_POST["id_reg"])." and id_cust_usr = ".QuoteValue(DPE_NUMERIC,$_POST["id_cust_usr"])."
				and reg_ina_jenis = ".QuoteValue(DPE_CHAR,STATUS_INAP_PEMERIKSAAN);
          $rs = $dtaccess->Execute($sql);
		$dataKlaim = $dtaccess->Fetch($rs);
		...
		$_POST["rawat_ina_klaim_kode"][0] = $dataKlaim["ina_kode"];
          ...
          $sql = "select item_id, item_nama,id_petunjuk,id_rawat from klinik.klinik_perawatan_terapi a
                    left join logistik.logistik_item b on a.id_rawat = b.item_id
...
//
		...
		$_POST["op_selesai_menit"] = $tmpJamSelesai[1];
          ...
          $_POST["rawat_suster_nama"] = $row["pgw_nama"];
		...
		//-- pencarian data di tabel preop --//
				...
				join klinik.klinik_refraksi c on a.reg_id = c.id_reg 
                    ...
                    where a.reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
          $dataPasien3= $dtaccess->Fetch($sql);
          
        //  $pemeriksaanLink = "perawatan_perawat_visite.php?id_cust_usr=".$enc->Encode($dataPasien3["id_cust_usr"])."&id_reg=".$enc->Encode($_GET["id_reg"]);   
          $diagLink = "perawatan_diag.php?id_cust_usr=".$enc->Encode($dataPasien3["id_cust_usr"])."&id_reg=".$enc->Encode($_GET["id_reg"]);
	...
	if ($_POST["btnSaveRawat"] || $_POST["btnUpdateRawat"]) {
          ...
          $dbField[65] = "rawat_flag"; 
          $dbField[66] = "rawat_flag_komen";  
                    
          if($_POST["btnSaveRawat"]) $dbField[67] = "rawat_waktu";
          ...
          $dbValue[65] = QuoteValue(DPE_CHAR,'I'); 
          $dbValue[66] = QuoteValue(DPE_CHAR,'RAWAT INAP'); 
           
          if($_POST["btnSaveRawat"]) $dbValue[67] = QuoteValue(DPE_DATE,date("Y-m-d H:i:s"));
		...
		$dtaccess->Execute($sql);
          ...
          if($_POST["reg_jenis_pasien"]==PASIEN_BAYAR_JAMKESNAS_PUSAT) {
			...
			$dbField[7] = "reg_ina_who";
				...
				$dbValue[5] = QuoteValue(DPE_CHAR,STATUS_INAP_PEMERIKSAAN);
...
$sql = "select a.obat_harga_jual, b.terapi_jumlah_item from logistik.logistik_item a
        left join klinik.klinik_perawatan_terapi b on a.item_id=b.id_item
          ...
          $dbField[10] = "fol_nominal_satuan";
               ...
               $dbValue[8] = QuoteValue(DPE_CHARKEY,$_POST["item_id"][$i]);
          ...
          if($_POST["_x_mode"]!="Edit") {
               ...
               if($_POST["cmbNext"]==STATUS_INAP_CEKOUT && $_POST["reg_jenis_pasien"]!=PASIEN_BAYAR_SWADAYA) { 
           ...
           //---End update status bed---//    
                    ...
                    $sql = "select b.paket_klaim_id, paket_klaim_total 
                              ...
                              where a.biaya_pasien_status = ".QuoteValue(DPE_CHAR,STATUS_INAP_PEMERIKSAAN)."
                    ...
                    while($row = $dtaccess->Fetch($rs)) {
                         ...
                         $dbField[6] = "reg_klaim_jenis";
                              ...
                              $dbValue[6] = QuoteValue(DPE_CHAR,STATUS_INAP_PEMERIKSAAN);
               ...
               if(!$dataDiag) {
				...
				$sql = "delete from klinik.klinik_folio where id_reg = ".QuoteValue(DPE_CHAR,$_POST["id_reg"])."
						and id_cust_usr = ".QuoteValue(DPE_NUMERIC,$_POST["id_cust_usr"])." and fol_jenis = '".STATUS_INAP_PEMERIKSAAN."'";
				$dtaccess->Execute($sql);
				...
				for($i=0,$n=count($dataBiaya);$i<$n;$i++) {
				  if($dataBiaya[$i]["biaya_jenis"]=='D') $dataBiaya[$i]["biaya_jenis"] = 'ID';
	...
	//-- END OF PROSES PENYIMPANAN UNTUK TAB RAWAT / PEMERIKSAAN --//
            ...
            $sql = "select b.ina_kode, b.ina_nama
                    from klinik.klinik_perawatan_ina a join klinik.klinik_ina b on a.id_ina = b.ina_id
                    where a.id_rawat = ".QuoteValue(DPE_CHAR,$_POST["rawat_id"])."
                    and a.rawat_ina_odos = 'OD' 
                    order by a.rawat_ina_urut";
          $dataDiagInaOD2 = $dtaccess->FetchAll($sql);	
	//echo $sql;
	...
	//-- PROSES PENYIMPANAN UNTUK TAB DIAGNOSTIK --//
		if ($_POST["btnSaveDiag"] || $_POST["btnUpdateDiag"]) {
          ...
          if($_POST["id_dokter"]) {
               ...
               $dbValue[0] = QuoteValue(DPE_CHAR,$dtaccess->GetTransID());
               $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["diag_id"]);
               $dbValue[2] = QuoteValue(DPE_NUMERICKEY,$_POST[$i]["id_dokter"]);
	...
	if ($_POST["btnSavePreop"] || $_POST["btnUpdatePreop"]) {
		...
		if($_POST["btnSavePreop"]) {
          ...
          $dtaccess->Execute($sql,DB_SCHEMA_KLINIK);
               ...
               $sql = "delete from klinik.klinik_folio where id_reg = ".QuoteValue(DPE_CHAR,$_POST["id_reg"])."
                         and id_cust_usr = ".QuoteValue(DPE_NUMERIC,$_POST["id_cust_usr"])." and fol_jenis = '".STATUS_INAP_PREOP."'";
               $dtaccess->Execute($sql);
               ...
               for($i=0,$n=count($dataBiaya);$i<$n;$i++) {
                    ...
                    $dbValue[3] = QuoteValue(DPE_NUMERIC,$dataBiaya[$i]["biaya_total"]);
                    $dbValue[4] = QuoteValue(DPE_CHAR,$dataBiaya[$i]["biaya_id"]);
                    $dbValue[5] = QuoteValue(DPE_CHAR,STATUS_INAP_PREOP);
                    $dbValue[6] = QuoteValue(DPE_NUMERICKEY,$_POST["id_cust_usr"]);
                    $dbValue[7] = QuoteValue(DPE_DATE,$folWaktu);
                    $dbValue[8] = QuoteValue(DPE_CHAR,$lunas);
                    $dbValue[9] = QuoteValue(DPE_NUMERIC,'1');
		 ...
		 // --- insrt suster ---
          ...
          if($_POST["id_dokter"]) {
               ...
               $dbValue[0] = QuoteValue(DPE_CHAR,$dtaccess->GetTransID());
               $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["preop_id"]);
               $dbValue[2] = QuoteValue(DPE_NUMERICKEY,$_POST["id_dokter"][0]);
	...
	//-- PROSES PENYIMPANAN UNTUK TAB BEDAH --//
	 if($_POST["btnSaveOP"] || $_POST["btnUpdateOP"]) {
          ...
          $dbField[11] = "op_paket_biaya";
         // $dbField[12] = "id_suster";
          ...
          $dbValue[9] = QuoteValue(DPE_NUMERICKEY,$_POST["id_dokter"][0]);
          $dbValue[10] = QuoteValue(DPE_NUMERICKEY,$_POST["id_cust_usr"]);
          $dbValue[11] = QuoteValue(DPE_CHARKEY,$_POST["op_paket_biaya"]);
          //$dbValue[12] = QuoteValue(DPE_NUMERICKEY,$_POST["id_suster"]);
		...
		$dtaccess->Execute($sql); 
          ...
          /*if($_POST["btnSaveOP"]) {
			...
			$sql = "delete from klinik.klinik_folio where id_reg = ".QuoteValue(DPE_CHAR,$_POST["id_reg"])."
					and fol_jenis = ".QuoteValue(DPE_CHAR,STATUS_INAP_BEDAH);
			$dtaccess->Execute($sql);
      ...
      $dbField[9] = "fol_nominal_satuan";
				...
				$dbValue[5] = QuoteValue(DPE_CHAR,STATUS_INAP_BEDAH);
				$dbValue[6] = QuoteValue(DPE_NUMERICKEY,$_POST["id_cust_usr"]);
				$dbValue[7] = QuoteValue(DPE_DATE,$folWaktu); 
			...
			$dbValue[9] = QuoteValue(DPE_NUMERIC,$dataBiaya["op_paket_total"]);
               ...
               if($_POST["reg_jenis_pasien"]!=PASIEN_BAYAR_SWADAYA) { 
                    ...
                    $sql = "select b.paket_klaim_id, paket_klaim_total 
                              from klinik.klinik_biaya_pasien a
                              join klinik.klinik_paket_klaim b on a.id_paket_klaim = b.paket_klaim_id  
                              where a.biaya_pasien_status = ".QuoteValue(DPE_CHAR,STATUS_INAP_BEDAH)."
                              and a.biaya_pasien_jenis = ".QuoteValue(DPE_CHAR,$_POST["reg_jenis_pasien"]);
                    $rs = $dtaccess->Execute($sql);
                    ...
                    while($row = $dtaccess->Fetch($rs)) {
                         ...
                         $dbField[6] = "reg_klaim_jenis";
                              ...
                              $dbValue[4] = QuoteValue(DPE_DATE,date("Y-m-d H:i:s"));
                              $dbValue[5] = QuoteValue(DPE_CHAR,$userData["name"]);
                              $dbValue[6] = QuoteValue(DPE_CHAR,STATUS_INAP_BEDAH);
			...
			if($_POST["reg_jenis_pasien"]==PASIEN_BAYAR_JAMKESNAS_PUSAT && $_POST["id_ina"]){
                    ...
                    $sql = "delete from klinik.klinik_registrasi_ina
                              where id_reg = ".QuoteValue(DPE_CHAR,$_POST["id_reg"])."
                              and reg_ina_jenis = ".QuoteValue(DPE_CHAR,STATUS_INAP_OPERASI);
                    $rs = $dtaccess->Execute($sql);                    
          			...
          			$dbField[4] = "id_cust_usr";
                         ...
                         $dbField[8] = "reg_ina_who";
          				...
          				$dbValue[4] = QuoteValue(DPE_CHAR,$_POST["id_cust_usr"]);
                              ...
                              $dbValue[6] = QuoteValue(DPE_CHAR,STATUS_INAP_OPERASI);
                              $dbValue[7] = QuoteValue(DPE_DATE,date("Y-m-d H:i:s"));
          ...
          $sql = "update klinik.klinik_registrasi set reg_status = ".QuoteValue(DPE_CHAR,$_POST["cmbNext"])." 
                  where reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
          $dtaccess->Execute($sql,DB_SCHEMA);
          
          if($_POST["cmbNext"]==STATUS_INAP_CEKOUT){
          
       //****** BED Buka ******//
    
     $sql = "select rawatinap_tanggal_masuk, current_date, rawatinap_tanggal_keluar from klinik.klinik_rawatinap where id_reg = ".QuoteValue(DPE_DATE,$_GET["id_reg"]);
     $dataWaktu = $dtaccess->Fetch($sql);      
       
     $totalWaktu = DateDiff($dataWaktu["rawatinap_tanggal_masuk"],$dataWaktu["date"]);
      if($totalWaktu <= 0) $totalWaktu = 1;
     $sql="select biaya_id,biaya_nama, biaya_total,biaya_jenis from klinik.klinik_biaya where biaya_jenis = ".QuoteValue(DPE_CHAR,$_POST["id_kategori_kamar"]);
     $dataKamarHari = $dtaccess->Fetch($sql);
    
     
     $totalWaktuHari = $totalWaktu * $dataKamarHari["biaya_total"];
     
                         $dbTable = "klinik.klinik_kamar_bed";
               
                              $dbField[0] = "bed_id";   // PK
                              $dbField[1] = "id_kamar";
                              $dbField[2] = "bed_reserved";
                                     
                                   $dbValue[0] = QuoteValue(DPE_CHAR,$_POST["bed_id"]);
                                   $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["id_kamar"]);
                                   $dbValue[2] = QuoteValue(DPE_CHAR,'n');
        ...
        $dbField[10] = "fol_nominal_satuan";   
               ...
               $dbValue[0] = QuoteValue(DPE_CHARKEY,$folId);
               $dbValue[1] = QuoteValue(DPE_CHARKEY,$_POST["id_reg"]);
               $dbValue[2] = QuoteValue(DPE_CHARKEY,$dataKamarHari["biaya_nama"]);
               $dbValue[3] = QuoteValue(DPE_NUMERIC,StripCurrency($totalWaktuHari));
               $dbValue[4] = QuoteValue(DPE_CHARKEY,$dataKamarHari["biaya_jenis"]);
               $dbValue[5] = QuoteValue(DPE_CHARKEY,$_POST["id_cust_usr"]);
               $dbValue[6] = QuoteValue(DPE_DATE,date("Y-m-d H:i:s"));
               $dbValue[7] = QuoteValue(DPE_CHARKEY,$lunas);
               $dbValue[8] = QuoteValue(DPE_CHARKEY,$dataKamarHari["biaya_id"]);
               $dbValue[9] = QuoteValue(DPE_NUMERIC,$totalWaktu);
               $dbValue[10] = QuoteValue(DPE_NUMERIC,StripCurrency($totalWaktuHari));
          ...
          $sql = "select * from klinik.klinik_biaya_split where id_biaya = ".QuoteValue(DPE_CHAR,$dataKamarHari["biaya_id"]);
					$splitData = $dtaccess->FetchAll($sql,DB_SCHEMA);
     ...
     //****** BED TUTUP ******//
          ...
          $dbField[0] = "id_reg";
          $dbField[1] = "rawatinap_tanggal_keluar";
          $dbField[2] = "rawatinap_waktu_keluar";
 
          $dbValue[0] = QuoteValue(DPE_CHAR,$_POST["id_reg"]);
        ...
        $dbTable = "klinik_registrasi";
          $dbField[0] = "reg_id";
          $dbField[1] = "reg_status";
 
          $dbValue[0] = QuoteValue(DPE_CHAR,$_POST["id_reg"]);
          $dbValue[1] = QuoteValue(DPE_CHAR,STATUS_INAP_CEKOUT);
	...
	foreach($rawatKeadaan as $key => $value) {
     ...
     $optionsNext[$count] = $view->RenderOption(STATUS_INAP_CEKOUT,"Pulang",$show); $count++;
	...
	for($i=0,$n=count($dataVisus);$i<$n;$i++) {
     ...
     $sql = "select item_id, item_nama from logistik.logistik_item 
               where id_kategori = '6787f9f458da266cd5e72b74e27b843b' 
               order by item_id";
     ...
     $('#tb_terapi').createAppend(
          'tr', { class  : 'tablecontent-odd',id:'tr_terapi_'+akhir+'' },
               ...
               'td', { align: 'center', style: 'color: black;' },
                    [
                         'input', {type:'text', value:'', size:35, maxLength:100,name:'petunjuk_nama[]', id:'petunjuk_nama_'+akhir+''}
                    ],
               'td', { align: 'center', style: 'color: black;' },
                    [
...
<?php if(!$_GET["id"]) { ?>
	<div id="antri_kanan" style="width:100%;">
		<div class="tableheader">Dokter Visite</div>
...
<tr>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
          <tr>
               ...
               <td width= "70%" align="left" class="tablecontent-odd"><label><?php echo $dataPasien2["cust_usr_kode"]; ?></label></td>
          </tr>	
          <tr>
               <td width= "30%" align="left" class="tablecontent">Nama Lengkap</td>
               <td width= "70%" align="left" class="tablecontent-odd"><label><?php echo $dataPasien2["cust_usr_nama"]; ?></label></td>
          </tr>
          <tr>
               <td width= "30%" align="left" class="tablecontent">Umur</td>
               <td width= "70%" align="left" class="tablecontent-odd"><label><?php echo $dataPasien2["umur"]; ?></label></td>
          </tr>
          <tr>
               <td width= "30%" align="left" class="tablecontent">Jenis Kelamin</td>
               <td width= "70%" align="left" class="tablecontent-odd"><label><?php echo $jenisKelamin[$dataPasien2["cust_usr_jenis_kelamin"]]; ?></label></td>
          </tr>
          <tr>
  ...
  <!--   <fieldset>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1" id="tb_terapi"> 
          ...
          <?php if(!$_POST["item_nama"]) { ?>
               <tr  class="tablecontent-odd" id="tr_terapi_0">
                    <td align="left" class="tablecontent-odd">
                         ...
                         <input type="hidden" name="item_id[0]" id="item_id_0" value="<?php echo $_POST["obat_id"][0]?>" />                    
                    </td>
                    <td align="left" class="tablecontent-odd">
                    <input readonly size="35" maxlength="100" name="petunjuk_nama[0]" id="petunjuk_nama_0" value="<?php echo $_POST["petunjuk_nama"][0]?>" />
                    </td>
          ...
          <?php } else { ?>
               <?php for($i=0,$n=count($_POST["obat_id"]);$i<$n;$i++) { ?>
                    <tr id="tr_terapi_<?php echo $i;?>">
                         <td align="left" class="tablecontent-odd">
                              ...
                              <input type="hidden" id="obat_id_<?php echo $i;?>" name="obat_id[]" value="<?php echo $_POST["obat_id"][$i];?>"/>
                         </td>
                         <td align="left" class="tablecontent-odd">
    ...
    <iframe style="width:100%;height:30%;" marginwidth="0" marginheight="0" id="ifrmDiag" name="ifrmDiag" src="<?php echo $pemeriksaanLink;?>" scrolling="auto" align="center" frameborder="0"></iframe>
  
    <iframe style="width:100%;height:100%;" marginwidth="0" marginheight="0" id="ifrmDiag" name="ifrmDiag" src="<?php echo $diagLink;?>" scrolling="auto" align="center" frameborder="0"></iframe>
    </td>
...
<input type="hidden" name="id_reg" value="<?php echo $cug;?>"/>
...
<tr>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
          <tr>
               ...
               <td align="left" class="tablecontent-odd" width="80%"> 
                    <?php echo $view->RenderTextBox("rawat_dokter_nama","rawat_dokter_nama","30","100",$_POST["rawat_dokter_nama"],"inputField", "readonly",false);?>
                    <a href="<?php echo $dokterPage;?>&TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Cari Dokter"><img src="<?php echo($APLICATION_ROOT);?>images/bd_insrow.png" border="0" align="middle" width="18" height="20" style="cursor:pointer" title="Cari Dokter" alt="Cari Dokter" /></a>
                    <input type="text" id="id_dokter" name="id_dokter" value="<?php echo $_POST["id_dokter"];?>"/>
               </td>
          </tr>
  ...
  <!--   <fieldset>
     ...
     <table width="60%" border="1" cellpadding="4" cellspacing="1">
          <tr>
               <td width="25%" align="left" class="tablecontent">Gambar USG I</td>
               <td width="25%" align="left" class="tablecontent">Gambar USG II</td>
               <td width="25%" align="left" class="tablecontent">Gambar USG III</td>
               <td width="25%" align="left" class="tablecontent">Gambar USG IV</td>
          </tr>
          <tr>
               <td align="center"  class="tablecontent-odd">
          ...
          <tr>
               <td align="center">
                    <button class="button" id="buttonUpload" onclick="return ajaxFileUpload('diag_usg.php','diag_gambar_usg','img_usg');">Upload USG I</button>
               </td>
               <td align="center">
                    <button class="button" id="buttonUpload" onclick="return ajaxFileUpload('diag_fundus.php','diag_gambar_fundus','img_fundus');">Upload USG II</button>
               </td>
               <td align="center">
                    <button class="button" id="buttonUpload" onclick="return ajaxFileUpload('diag_humpre.php','diag_gambar_humpre','img_humpre');">Upload USG III</button>
               </td>
               <td align="center">
                    <button class="button" id="buttonUpload" onclick="return ajaxFileUpload('diag_oct.php','diag_gambar_oct','img_oct');">Upload USG IV</button>
               </td>
          </tr>
	</table>
     </fieldset> -->
...
<tr>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
          <tr>
               ...
               <td width= "80%" align="left" class="tablecontent-odd"><label><?php echo $dataPasien2["cust_usr_kode"]; ?></label></td>
          </tr>	
          <tr>
               <td width= "20%" align="left" class="tablecontent">Nama Lengkap</td>
               <td width= "80%" align="left" class="tablecontent-odd"><label><?php echo $dataPasien2["cust_usr_nama"]; ?></label></td>
          </tr>
          <tr>
               <td width= "20%" align="left" class="tablecontent">Umur</td>
               <td width= "80%" align="left" class="tablecontent-odd"><label><?php echo $dataPasien2["umur"]; ?></label></td>
          </tr>
          <tr>
               <td width= "20%" align="left" class="tablecontent">Jenis Kelamin</td>
               <td width= "80%" align="left" class="tablecontent-odd"><label><?php echo $jenisKelamin[$dataPasien2["cust_usr_jenis_kelamin"]]; ?></label></td>
          </tr>
	</table>
     </fieldset>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
          <tr>
               <td align="left" class="tablecontent" width="20%">Diagnosis 1</td>
               <td align="left" class="tablecontent-odd"><?php echo $dataDiagInaOD2[0]["ina_kode"]." ".$dataDiagInaOD2[0]["ina_nama"];?></td>
          </tr>
          <tr>
               <td align="left" class="tablecontent" width="20%">Diagnosis 2</td>
               <td align="left" class="tablecontent-odd"><?php echo $dataDiagInaOD2[1]["ina_kode"]." ".$dataDiagInaOD2[1]["ina_nama"];?></td>
          </tr>
	</table>
     </fieldset>
...
<tr>
				...
				<table width="100%" border="1" cellpadding="1" cellspacing="1" id="tb_suster_op">
					...
					<tr id="tr_suster_op_0">
						<td align="left" class="tablecontent-odd" width="40%">
							<?php echo $view->RenderTextBox("rawat_suster_nama[]","rawat_suster_nama_0","30","100",$_POST["rawat_suster_nama"][0],"inputField", "readonly",false);?>
							<a href="<?php echo $susterPage;?>&el=0&TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Cari Suster"><img src="<?php echo($APLICATION_ROOT);?>images/bd_insrow.png" border="0" align="middle" width="18" height="20" style="cursor:pointer" title="Cari Suster" alt="Cari Suster" /></a>
							<input type="hidden" id="id_suster_0" name="id_suster_[]" value="<?php echo $_POST["id_suster_"];?>"/>
			               </td>
                    ...
                    <?php } else  { ?>
                         <?php for($i=0,$n=count($_POST["id_suster_"]);$i<$n;$i++) { ?>
                              <tr id="tr_suster_op_<?php echo $i;?>">
								...
								<?php if($edit) { ?>
									<a href="<?php echo $susterPage;?>&el=<?php echo $i;?>&TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Cari Suster"><img src="<?php echo($APLICATION_ROOT);?>images/bd_insrow.png" border="0" align="middle" width="18" height="20" style="cursor:pointer" title="Cari Suster" alt="Cari Suster" /></a>
								<?php } ?>
                                        <input type="hidden" id="id_suster_<?php echo $i;?>" name="id_suster_[]" value="<?php echo $_POST["id_suster_"][$i];?>"/>
                                   </td>
                                   <td align="left" class="tablecontent-odd" width="30%">
								<?php if($edit) { ?>
									<?php if($i==0) { ?>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
          <tr>
               <td align="left" width="20%" class="tablecontent">Dokter</td>
               <td align="left" class="tablecontent-odd" width="30%"> 
                    <?php echo $view->RenderTextBox("rawat_dokter_nama","rawat_dokter_nama","20","100",$_POST["rawat_dokter_nama"],"inputField", "readonly",false);?>
                    <a href="<?php echo $dokterPage;?>&TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Cari Dokter"><img src="<?php echo($APLICATION_ROOT);?>images/bd_insrow.png" border="0" align="middle" width="18" height="20" style="cursor:pointer" title="Cari Dokter" alt="Cari Dokter" /></a>
                    <input type="hidden" id="id_dokter" name="id_dokter" value="<?php echo $_POST["id_dokter"];?>"/>
               </td>
...
<input type="hidden" name="bed_id" value="<?php echo $_POST["bed_id"];?>"/>
<input type="hidden" name="id_kamar" value="<?php echo $_POST["id_kamar"];?>"/>
<input type="hidden" name="id_kategori_kamar" value="<?php echo $_POST["id_kategori_kamar"];?>"/>
