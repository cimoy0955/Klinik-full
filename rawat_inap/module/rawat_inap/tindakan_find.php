...
function GetData($in_nama,$in_total){
	...
	$sql_where[] = "biaya_nama notnull"; 
	if($in_nama) $sql_where[] = "UPPER(biaya_nama) like ".QuoteValue(DPE_CHAR,strtoupper("%".$in_nama."%"));
	if($in_total) $sql_where[] = "UPPER(biaya_total) like ".QuoteValue(DPE_CHAR,strtoupper("%".$in_total."%"));
	$sql_where = implode(" and ",$sql_where);

	// --- cari data krsnya ---
	$sql = "select biaya_id, biaya_nama, biaya_total,biaya_jenis,  biaya_id from klinik.klinik_biaya a";
	$sql .= " where a.biaya_jenis like '%T%' and ".$sql_where;			
	$sql .= " order by a.biaya_total";
...
*/
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$counter++;
		*/
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["biaya_id"].'\',\''.$dataTable[$i]["biaya_total"].'\',\''.$dataTable[$i]["biaya_jenis"].'\',\''.addslashes(htmlentities($dataTable[$i]["biaya_nama"])).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,total,jenis,nama) {
		
	self.parent.document.getElementById('biaya_nama_<?php echo $_GET["el"];?>').value = nama;
		self.parent.document.getElementById('biaya_jenis_<?php echo $_GET["el"];?>').value = jenis;
	self.parent.document.getElementById('biaya_total_<?php echo $_GET["el"];?>').value = total;
	self.parent.document.getElementById('biaya_id_<?php echo $_GET["el"];?>').value = id;
