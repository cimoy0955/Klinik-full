...
*/
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$counter++;
		*/
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["biaya_id"].'\',\''.addslashes(htmlentities($dataTable[$i]["biaya_nama"])).'\',\''.$dataTable[$i]["biaya_total"].'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,nama,hrg) {
		
	self.parent.document.getElementById('biaya_nama').value = nama;
	self.parent.document.getElementById('txtHargaSatuan').value = hrg;
	self.parent.document.getElementById('txtJumlah').value = 1;
	self.parent.document.getElementById('biaya_id').value = id;
	self.parent.document.getElementById('txtHargaTotal').value = hrg;
