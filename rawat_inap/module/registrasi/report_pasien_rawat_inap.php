<?php
    ...
    $cetakPage = "report_pengaduan_cetak.php?tanggal_awal="
     ...
     $sql = "select b.cust_usr_kode, b.cust_usr_nama, b.cust_usr_alamat, b.cust_usr_tanggal_lahir, b.cust_usr_jenis_kelamin, 
               a.reg_jenis_pasien, a.reg_status_pasien, a.reg_waktu, c.rawatinap_id, c.rawatinap_tanggal_masuk, c.rawatinap_tanggal_keluar, c.rawatinap_waktu_masuk , c.rawatinap_waktu_keluar,
                d.kamar_nama, e.kategori_nama , e.kategori_harga, f.bed_kode
     ...
     if($sql_where) $sql .= " where ".implode(" and ",$sql_where)."and rawatinap_id <> 'not null'";
  ...
  $counter++;
	...
	$tbHeader[0][$counter][TABLE_ISI] = "Tanggal Keluar";
	$tbHeader[0][$counter][TABLE_WIDTH] = "15%";
  $counter++;
	...
	$tbHeader[0][$counter][TABLE_ISI] = "Kategori Kamar";
  ...
  for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
	  ...
	  $tbContent[$i][$counter][TABLE_ISI] = format_date_long($dataTable[$i]["rawatinap_tanggal_masuk"]);
		...
		$tbContent[$i][$counter][TABLE_ISI] = format_date_long($dataTable[$i]["rawatinap_tanggal_keluar"]);
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$counter++;
