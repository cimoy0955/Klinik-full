<?php
	...
	if($_POST["btnLanjut"]) {
		$sql = "select a.*,b.cust_nama,reg_jenis_pasien,c.*  from global.global_customer_user a
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
		...
		$sql = "select reg_status,reg_id from klinik.klinik_registrasi 
		...
		$sql = "select preop_tanggal_jadwal 
          ...
          $dbValue[4] = QuoteValue(DPE_CHAR,(STATUS_PEMERIKSAAN.STATUS_ANTRI));
          ...
          $dbValue[12] = QuoteValue(DPE_CHAR,$_POST["reg_ugd"]);
          $dbValue[13] = QuoteValue(DPE_NUMERIC,$_POST["id_poli"]);
          ...
          if($dataReg["reg_status"]{0}==STATUS_SELESAI || !$dataReg) { 
              ...
              // $sql = "select reg_id from klinik.klinik_registrasi where reg_id = ".QuoteValue(DPE_CHAR,$regId);
               //$dataReg = $dtaccess->Fetch($sql,DB_SCHEMA);
     ...
     elseif($dataReg["reg_status"]{0}==STATUS_SELESAI || $dataReg || $_POST["cust_usr_id"]) { 
               $sql = "select * from klinik.klinik_biaya where biaya_id = ".QuoteValue(DPE_CHAR,BIAYA_LOKET);
               $dataBiaya = $dtaccess->Fetch($sql,DB_SCHEMA);  
     ...
     // --- cari agama ---
     $sql = "select distinct(agm_nama),agm_id from global.global_agama order by agm_id";
     $rs = $dtaccess->Execute($sql,DB_SCHEMA_HRIS);
     $dataAgama = $dtaccess->FetchAll($rs);

     // --- cari poli ---
     $sql = "select distinct(poli_nama),poli_id from global.global_auth_poli order by poli_id";
...
<?php if($_POST["cust_usr_id"] || $_POST["btnAdd"]) { ?>
...
<table width="100%" border="1" cellpadding="4" cellspacing="1">
	...
	<tr>
		<td class="tablecontent">Penanganan</td>
		<td colspan="2" class="tablecontent-odd">
			<select name="reg_ugd" onKeyDown="return tabOnEnter(this, event);">
				<option value="n" <?php if($_POST["reg_ugd"]=="n")echo "selected";?>>Biasa</option>
				<option value="y" <?php if($_POST["reg_ugd"]=="y")echo "selected";?>>UGD</option>
			</select>
          </td>
	</tr>
	<tr>
		<td class="tablecontent">Poli</td>
          <td colspan="2" class="tablecontent-odd">
