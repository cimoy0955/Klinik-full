...
//echo $_POST["cust_usr_kode"];
	...
	if($_POST["cust_usr_kode"]) {
		$sql = "select cust_usr_id,cust_usr_nama,cust_usr_kode, b.cust_usr_jenis_kelamin, ((current_date - cust_usr_tanggal_lahir)/365) as umur 
	     ...
	     $sql = "select * from klinik.klinik_perawatan where rawat_flag = 'I' and id_cust_usr = ".QuoteValue(DPE_CHAR,$dataPasien["cust_usr_id"])." and id_reg <> ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
		...
		for($i=0,$n=count($dataPemeriksaan);$i<$n;$i++) {
			...
			$sql = "select obat_nama, petunjuk_nama from klinik.klinik_perawatan_terapi a
						left join apotik.apotik_obat_master b on a.id_item = b.obat_id
						left join apotik.apotik_obat_petunjuk c on b.id_petunjuk = c.petunjuk_id where id_rawat = ".QuoteValue(DPE_CHAR,$dataPemeriksaan[$i]["rawat_id"]);
			$rs = $dtaccess->Execute($sql);

			
			$j=0; unset($row);
			while($row=$dtaccess->Fetch($rs)) {
				$dataPemeriksaan[$i]["terapi_obat"][$j] = $row["obat_nama"];
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" id="noborder">
	<tr>
		<td align="left" colspan=2 class="tableheader" class="noborder">Rekam Medik Rawat Inap</td>
	</tr> 
</table> 
...
<?php for($i=0,$n=count($dataPemeriksaan);$i<$n;$i++) { ?> 
	<fieldset style="border-color:red"> 
	  <?php if($dataPemeriksaan[$i]["rawat_waktu"]) { ;?>
     <legend><strong><?php echo format_date(substr($dataPemeriksaan[$i]["rawat_waktu"],0,10));?></strong></legend> 
    <?php } else { ;?>
     <legend><strong><?php echo format_date($dataPemeriksaan[$i]["rawat_tanggal"])."&nbsp;".$dataPemeriksaan[$i]["rawat_waktu_kontrol"];?></strong></legend> 
    <?php } ;?>  
    ...
    <legend><strong>Diagnosa Keperawatan</strong></legend>
     <table width="100%" border="1" cellpadding="4" cellspacing="1"> 
          <tr>
               ...
               <td align="left" class="tablecontent-odd"><?php echo nl2br($dataPemeriksaan[$i]["rawat_diagnostik_perawat"]);?></td>
          </tr>
     </table>
    ...
    <legend><strong>Observasi</strong></legend>
     <table width="100%" border="1" cellpadding="4" cellspacing="1"> 
          <tr>
               ...
               <td align="left" class="tablecontent-odd"><?php echo nl2br($dataPemeriksaan[$i]["rawat_observasi"]);?></td>
          </tr>
     </table>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1" id="tb_tindakan"> 
          <tr class="subheader">
               <td width="30%" align="center">Jenis Tindakan</td>
               <td width="30%" align="center">Dosis</td>
          </tr>	
		<?php for($j=0,$k=count($dataPemeriksaan[$i]["terapi_obat"]);$j<$k;$j++) { ?>
			<tr  class="tablecontent-odd" id="tr_terapi_0">
