<?php
	...
	if($_POST["cust_usr_id"]) {
     ...
     if($dataPasien) {
          ...
          $sql = "select a.preop_id,  CAST(preop_waktu as date) as preop_tanggal from klinik.klinik_preop a where a.id_cust_usr = ".QuoteValue(DPE_NUMERIC,$_POST["cust_usr_id"])." 
                    order by a.preop_waktu desc";
          ...
          $isAllowedUpdate = $auth->IsAllowed("operasi",PRIV_UPDATE);
          ...
          for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
               ...
               if($isAllowedUpdate) {
                    $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataTable[$i]["preop_id"]).'"><img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/b_edit.png" alt="Edit" title="Edit" border="0"></a>';               
                    $tbContent[$i][$counter][TABLE_ALIGN] = "center";
                    $counter++;
               }
               
               $tbContent[$i][$counter][TABLE_ISI] = format_date($dataTable[$i]["preop_tanggal"]);
...
<table width="100%" border="0" cellpadding="4" cellspacing="1">
	<tr>
		<td align="left" colspan=2 class="tableheader">Data PreOp</td>
	</tr>
</table> 
