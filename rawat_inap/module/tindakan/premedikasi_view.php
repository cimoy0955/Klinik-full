<?php
	...
	$tree = new CTree("global.global_customer","cust_id",TREE_LENGTH);
     ...
     $_x_mode = "New";
     $thisPage = "premedikasi_view.php";
     $editPage = "premedikasi.php?";
	...
	if($_POST["cust_usr_id"]) {
     ...
     if($dataPasien) {
          ...
          for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
               ...
               if($isAllowedUpdate) {
                    $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataTable[$i]["preme_id"]).'"><img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/b_edit.png" alt="Edit" title="Edit" border="0"></a>';               
                    $tbContent[$i][$counter][TABLE_ALIGN] = "center";
                    $counter++;
               }
               
               $tbContent[$i][$counter][TABLE_ISI] = format_date($dataTable[$i]["preme_tanggal"]);
...
<table width="100%" border="0" cellpadding="4" cellspacing="1">
	<tr>
		<td align="left" colspan=2 class="tableheader">Data Premedikasi</td>
	</tr>
</table> 
