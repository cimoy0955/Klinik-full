<?php
   ...
   //  } elseif($auth->IsAllowed("setup_role",PRIV_READ)===1){
	...
	if ($_POST["id_rol_enc"]) $idRolEnc = $_POST["id_rol_enc"]; 
     ...
     $sql = "select a.* from global_auth_privilege a 
             left join global_auth_role_priv b 
             on a.priv_id = b.id_priv
             and b.id_rol = ".$_POST["id_rol"]." 
             where b.id_priv is null order by a.priv_name";
     $rs = $dtaccess->Execute($sql);
     $dataTable = $dtaccess->FetchAll($rs);
 
     $sql = "select rol_name from global_auth_role where rol_id = ".$_POST["id_rol"]." order by rol_name";
     $rs = $dtaccess->Execute($sql);
