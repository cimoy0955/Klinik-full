<?php
     ...
     $view = new CView($_SERVER["PHP_SELF"],$_SERVER['QUERY_STRING']);
	   ...
	   if(!$auth->IsAllowed("pos_cashflow_harian",PRIV_READ)){
          die("access_denied");
         exit(1);
          
    } elseif($auth->IsAllowed("pos_cashflow_harian",PRIV_READ)===1){
  ...
  $counter++;
	...
	for($i=0,$m=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$m++,$counter=0){
     ...
     if ($dataTable[$i]["penjualan_tipe"]=='T') $tipeBayar="TUNAI";
	  ...
	  $tbContent[$m][$counter][TABLE_ISI] = FormatTimestamp($dataTable[$i]["penjualan_create"]);
		...
		$tbContent[$m][$counter][TABLE_ISI] = $dataTable[$i]["penjualan_customer"];
		$tbContent[$m][$counter][TABLE_ALIGN] = "center";
		$counter++;
		...
		$tbContent[$m][$counter][TABLE_ISI] = currency_format($dataTable[$i]["penjualan_ppn"]);
		$tbContent[$m][$counter][TABLE_ALIGN] = "right";
		$counter++;
		
		if ($_POST["penjualan_tipe"] == "--") {
		$tbContent[$m][$counter][TABLE_ISI] = $tipeBayar;
		$tbContent[$m][$counter][TABLE_ALIGN] = "center";
		$counter++; }
		
		if ($_POST["id_petugas"] == "--") {
		$tbContent[$m][$counter][TABLE_ISI] = $dataTable[$i]["penjualan_petugas"];
		$tbContent[$m][$counter][TABLE_ALIGN] = "center";
		$counter++; }
		
		if ($_POST["id_dep"] == "--") {
		$tbContent[$m][$counter][TABLE_ISI] = $dataTable[$i]["dep_nama"];
		$tbContent[$m][$counter][TABLE_ALIGN] = "center";
		$counter++; }
		$j=0;$m++;$counter=0;
    }
    ...
    $tbContent[$m][$counter][TABLE_ISI] = "&nbsp;".currency_format($dataTable[$i]["transaksi_harga_jual"]);
		...
		$tbContent[$m][$counter][TABLE_ISI] = "&nbsp;".currency_format($dataTable[$i]["transaksi_total"]);
		$tbContent[$m][$counter][TABLE_ALIGN] = "left";
		$counter++;
