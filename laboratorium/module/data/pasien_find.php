...
function GetData($in_nama,$in_kode,$in_alamat,$in_umur,$in_jk){
	...
	// --- cari data krsnya ---
	$sql = "select cust_usr_id, cust_usr_nama, cust_usr_kode , cust_usr_alamat, pemeriksaan_create, pemeriksaan_id
			from laboratorium.lab_pemeriksaan a
			left join global.global_customer_user b on b.cust_usr_id = a.id_cust_usr";
	$sql .= " where ".$sql_where;			
	$sql .= " order by a.pemeriksaan_create";
	...
	$tbHeader[0][$counter][TABLE_ISI] = "Tanggal Periksa";
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tglPeriksa = explode(" ",$dataTable[$i]["pemeriksaan_create"]);
		$tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".format_date($tglPeriksa[0])."&nbsp;&nbsp;".$tglPeriksa[1];
		$tbContent[$i][$counter][TABLE_ALIGN] = "left";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
		
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["pemeriksaan_id"].'\',\''.$dataTable[$i]["cust_usr_kode"].'\')"/>';
...
function sendValue(id,kode) {
	self.parent.document.getElementById('pemeriksaan_id').value = id;
