<?php
     ...
     $plx = new InoLiveX("GetDayAJAX");
     
     function GetDayAJAX($month) {
	...
	$sql = "select * from klinik.klinik_split order by split_id";
     ...
     if($_POST["tanggal"] && $_POST["tanggal"]!="--") $sql_where[] = "extract(day from a.fol_dibayar_when) = '".$_POST["tanggal"]."'";
     if($_POST["bulan"] && $_POST["bulan"]!="--") $sql_where[] = "extract(month from a.fol_dibayar_when) = '".$_POST["bulan"]."'";
     if($_POST["tahun"]) $sql_where[] = "extract(year from a.fol_dibayar_when) = '".$_POST["tahun"]."'";
     if($_POST["fol_jenis"]) $sql_where[] = "a.fol_jenis = ".QuoteValue(DPE_CHAR,$_POST["fol_jenis"]);
     
     
     $sql_where[] = "a.fol_dibayar is not null";
	...
	$counter++;
     ...
     $tableHeader = "Report Kasir per Kas";

	if($_POST["btnExcel"]){
          header('Content-Type: application/vnd.ms-excel');
          header('Content-Disposition: attachment; filename=report_pembayaran_loket_per_kas_'.$_POST["tgl_awal"].'.xls');
     }
...
function GetDayMonth(month) {
     ...
     document.getElementById("div_tanggal").innerHTML = GetDayAJAX(month,"type=r"); 
}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
     <tr class="tableheader">
          <td colspan="<?php echo (count($dataSplit)+2)?>">&nbsp;<?php echo $tableHeader;?></td>
     </tr>
</table>
...
<table align="center" border=0 cellpadding=2 cellspacing=1 width="100%" id="tblSearching">
     <tr class="tablecontent">
          ...
          <td width="35%">
              <select name="fol_jenis">
				<option value="">[ Semua Layanan Biaya ]</option>
				<?php foreach($biayaStatus as $key=>$value) {?>
					<option value="<?php echo $key;?>" <?php if($_POST["fol_jenis"]==$key) echo "selected";?>><?php echo $value;?></option>
				<?php } ?>
			</select>
          </td> 
     </tr>
     <tr class="tablecontent">
		<td>&nbsp;Jenis Pasien</td>
          <td colspan="3"><?php echo $bayarPasien[PASIEN_BAYAR_SWADAYA]?>
