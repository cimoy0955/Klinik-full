<?php
	...
	$sql_where = implode(" and ",$sql_where);
     ...
     $sql = "select a.fol_id, a.id_reg, c.cust_usr_nama, c.cust_usr_kode, a.fol_nominal, a.fol_dibayar, b.biaya_nama, a.fol_jenis, a.id_biaya_tambahan,
               CAST(a.fol_dibayar_when as DATE) as tanggal, extract(year from a.fol_dibayar_when) as tahun, b.biaya_id
	       ...
	       join klinik.klinik_registrasi d on d.reg_id = a.id_reg and a.id_cust_usr = d.id_cust_usr "; /*cast(b.biaya_id as varchar) = a.id_biaya */
	$sql .= " where ".$sql_where; 
	$sql .= " order by a.id_reg";
     $dataTable = $dtaccess->FetchAll($sql);
     for($x=0;$x<count($dataTable);$x++){
	  $biayaPasien[$dataTable[$x]["id_reg"]][$dataTable[$x]["biaya_id"]] =  $dataTable[$x]["fol_dibayar"];
	  if($dataTable[$x]["id_reg"]!=$dataTable[$x-1]["id_reg"]){
	       $kodePasien[$dataTable[$x]["id_reg"]] = $dataTable[$x]["cust_usr_kode"];
	       $namaPasien[$dataTable[$x]["id_reg"]] = $dataTable[$x]["cust_usr_nama"];
	       $tanggalPasien[$dataTable[$x]["id_reg"]] = $dataTable[$x]["tanggal"];
	  }
     }
...
//	} 
	...
	for($i=0,$n=count($dataSplit);$i<$n;$i++){
		$tbHeader[0][$i+4][TABLE_ISI] = $dataSplit[$i]["biaya_nama"]."<br />(Rp.&nbsp;".currency_format($dataSplit[$i]["biaya_total"]).")";
