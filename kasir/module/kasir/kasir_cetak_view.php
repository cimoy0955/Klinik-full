<?php
     ...
     $cetakUlang = "kasir_cetak_ulang.php";
     ...
     function GetFolio() {
          ...
          $skrg = date('Y-m-d');
          $sql = "select distinct a.id_reg, a.id_cust_usr, b.cust_usr_nama,a.fol_jenis,a.fol_waktu,id_biaya,a.fol_lunas,b.cust_usr_jenis
                  ...
                  where reg_status <> 'C' and date(fol_waktu) = ".QuoteValue(DPE_DATE,$skrg)." order by fol_waktu asc";
		...
		for($i=0,$n=count($dataTable);$i<$n;$i++) {
          ...
          for($i=0,$nomor=1,$n=count($data),$counter=0;$i<$n;$i++,$counter=0) {
			   ...
			   $editPage = "kasir_cetak_ulang.php?jenis=".$fol[$data[$i]][$i]."&id_reg=".$reg[$data[$i]]."&waktu=".$waktu[$data[$i]];
	...
	if($_GET["id_reg"]) {
		...
		$sql = "select a.reg_jenis_pasien, cust_usr_alamat, cust_usr_nama,cust_usr_kode,b.cust_usr_jenis_kelamin,b.cust_usr_alergi,
				((current_date - cust_usr_tanggal_lahir)/365) as umur,  a.id_cust_usr
                    ...
                    join global.global_customer_user b on a.id_cust_usr = b.cust_usr_id 
                    where a.reg_id = ".QuoteValue(DPE_CHAR,$_GET["id_reg"])." and fol_jenis =  ".QuoteValue(DPE_CHAR,$_GET["jenis"]);
