 ...
 //return $sql;
	...
	if($_GET["id_reg"]) {
      ...
      $_POST["reg_jenis_pasien"] = $dataPasien["reg_jenis_pasien"];  
		...
		$sql = "select id_biaya from klinik.klinik_folio
					where id_reg = ".QuoteValue(DPE_CHAR,$_GET["id_reg"]);
				$dataNostik = $dtaccess->FetchAll($sql);
				//print_r($dataNostik);
				
      for($i=0,$n=count($dataNostik);$i<$n;$i++) {
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_KERATOMETRI){
      $dataNostik["keratometri"] = BIAYA_KERATOMETRI;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_BIOMETRI){
      $dataNostik["biometri"] = BIAYA_BIOMETRI;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_GULA){
      $dataNostik["gula"] = BIAYA_GULA;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_USG){
      $dataNostik["usg"] = BIAYA_USG;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_EKG){
      $dataNostik["ekg"] = BIAYA_EKG;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_FUNDUS){
      $dataNostik["fundus"] = BIAYA_FUNDUS;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_OPTHALMOSCOPY){
      $dataNostik["opthalmoscopy"] = BIAYA_OPTHALMOSCOPY;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_OCT){
      $dataNostik["oct"] = BIAYA_OCT;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_YAG){
      $dataNostik["yag"] = BIAYA_YAG;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_ARGON){
      $dataNostik["argon"] = BIAYA_ARGON;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_HUMPREY){
      $dataNostik["humprey"] = BIAYA_HUMPREY;      
      }
      
      if($dataNostik[$i]["id_biaya"]==BIAYA_SLT){
...
where id_app = ".QuoteValue(DPE_NUMERIC,'5');		
   ...
   $count=0;	
	...
	$optionsNext[$count] = $view->RenderOption(STATUS_PEMERIKSAAN,"Ruang 2 - Pemeriksaan",$show); $count++;
	$optionsNext[$count] = $view->RenderOption(STATUS_BEDAH,"Ruang 3 - Bedah Minor",$show); $count++;
	$optionsNext[$count] = $view->RenderOption(STATUS_PREMEDIKASI,"Ruang 5 - Penjadwalan Operasi",$show); $count++;
	$optionsNext[$count] = $view->RenderOption(STATUS_PREOP,"Ruang 5 - Pre Operasi",$show); $count++;
...
<? if($dataNostik["slt"] == BIAYA_SLT) { ?>
          <tr>
               <td align="left" class="tablecontent">Laser LST</td>
               <td align="left" class="tablecontent-odd"><?php echo $view->RenderTextBox("diag_slt","diag_slt","30","50",$_POST["diag_slt"],"inputField", null,null);?></td>
          </tr>
          <? } ?>
...
<? if($dataNostik["humprey"] == BIAYA_HUMPREY) { ?>
     ...
     <table width="60%" border="1" cellpadding="4" cellspacing="1" id="tbFoto" style="display:none">
          <tr>
