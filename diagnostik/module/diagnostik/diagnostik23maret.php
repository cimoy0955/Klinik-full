<?php
     ...
     function GetDiagnostik($status) {
          ...
          $sql = "select cust_usr_nama,a.reg_id, a.reg_status, a.reg_waktu, a.reg_jadwal, z.fol_lunas 
				...
				join (
					...
					where fol_lunas = 'y' and fol_jenis = '".STATUS_DIAGNOSTIK."' 
				) z on a.reg_id = z.id_reg 
                    where a.reg_status like '%D%' order by reg_status desc, reg_tanggal asc, reg_waktu asc";
 ...
 //return $sql;
   ...
   $count=0;	
	...
	$optionsNext[$count] = $view->RenderOption(STATUS_PEMERIKSAAN,"Pemeriksaan",$show); $count++;
	$optionsNext[$count] = $view->RenderOption(STATUS_PREMEDIKASI,"Penjadwalan Operasi",$show); $count++;
