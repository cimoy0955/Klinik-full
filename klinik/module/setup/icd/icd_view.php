<?php
     ...
     $editPage = "icd_edit.php?jenis=".$_GET["jenis"];
     $thisPage = "icd_view.php?jenis=".$_GET["jenis"];
     ...
     $sql = "select a.* from klinik.klinik_icd a where a.icd_jenis = ".QuoteValue(DPE_CHAR,$_GET["jenis"])." 
               order by a.icd_nomor";
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
          if($isAllowedDel) {
               $tbContent[$i][$counter][TABLE_ISI] = '<input type="checkbox" name="cbDelete[]" value="'.$dataTable[$i]["icd_id"].'">';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
          }
          
          if($isAllowedUpdate) {
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataTable[$i]["icd_id"]).'"><img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/b_edit.png" alt="Edit" title="Edit" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
          }
