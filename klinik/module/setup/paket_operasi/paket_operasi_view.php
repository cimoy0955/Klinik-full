<?php
     ...
     $editPage = "paket_operasi_edit.php";
     $thisPage = "paket_operasi_view.php";
     ...
     $sql = "select a.*, b.op_paket_split_nominal,b.id_split
               from klinik.klinik_operasi_paket a
               left join klinik.klinik_operasi_paket_split b on b.id_op_paket = a.op_paket_id 
               order by a.op_paket_nama, id_split"; 
     $rs = $dtaccess->Execute($sql);
     $dataTable = $dtaccess->FetchAll($rs);
     
     $tot = -1;
     for($i=0,$n=count($dataTable);$i<$n;$i++) {
          if($dataTable[$i]["op_paket_id"]!=$dataTable[$i-1]["op_paket_id"]){
               $tot++;
               $data[$tot] = $dataTable[$i]["op_paket_id"];
          }
          $nama[$dataTable[$i]["op_paket_id"]] = $dataTable[$i]["op_paket_nama"]; 
          $total[$dataTable[$i]["op_paket_id"]] = $dataTable[$i]["op_paket_total"]; 
          $biaya[$dataTable[$i]["op_paket_id"]][$dataTable[$i]["id_split"]] = $dataTable[$i]["op_paket_split_nominal"]; 
     }
	...
	$sql = "select * from klinik.klinik_split order by split_id";
     ...
     //*-- config table ---*//
     $tableHeader = "&nbsp;Paket Operasi";
     
     $isAllowedDel = $auth->IsAllowed("setup_paket_operasi",PRIV_DELETE);
     $isAllowedUpdate = $auth->IsAllowed("setup_paket_operasi",PRIV_UPDATE);
     $isAllowedCreate = $auth->IsAllowed("setup_paket_operasi",PRIV_CREATE);
