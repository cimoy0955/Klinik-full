<?php
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
		...
		if($_POST["pgw_tanggal_masuk"]) {
          ...
          if ($_POST["btnSave"]) 
               $sql = "SELECT pgw_id FROM hris_pegawai WHERE UPPER(pgw_nip) = ".QuoteValue(DPE_CHAR,strtoupper($_POST["pgw_nama"]));
          else
		...
		/*if($_POST["pgw_tanggal_habis_sk"]) {
			if(check_date(date_db($_POST["pgw_tanggal_habis_sk"]))==true)$err_code = clearbit($err_code,16);
			else $err_code = setbit($err_code,16);
		} else $err_code = clearbit($err_code,16);

		if($_POST["pgw_tmt_pangkat"]) {
			if(check_date(date_db($_POST["pgw_tmt_pangkat"]))==true)$err_code = clearbit($err_code,17);
			else $err_code = setbit($err_code,17);
		} else $err_code = clearbit($err_code,17);*/

		/*if($_POST["pgw_tanggal_sk_akademik"]) {
			if(check_date(date_db($_POST["pgw_tanggal_sk_akademik"]))==true)$err_code = clearbit($err_code,19);
			else $err_code = setbit($err_code,19);
		} else $err_code = clearbit($err_code,19);

		if($_POST["pgw_tmt_akademik"]) {
			if(check_date(date_db($_POST["pgw_tmt_akademik"]))==true)$err_code = clearbit($err_code,20);
			else $err_code = setbit($err_code,20);
		} else $err_code = clearbit($err_code,20);*/
