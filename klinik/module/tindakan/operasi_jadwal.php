<?php
	...
	$page[STATUS_PREOP] = "preop.php";
     ...
     $plx = new InoLiveX("GetTindakan,SetTindakan,GetTonometri,GetDosis,SetNext1");
     ...
     function GetDosis($item,$id=null) {
		...
		$sql = "select item_fisik from inentori.inv_item where item_id = ".QuoteValue(DPE_NUMERIC,$item);
     ...
     function SetNext1($param,$tipe=RAWAT_JALAN) {
	  global $view;
          if($tipe==RAWAT_JALAN){
	       $count=0;
	       if($param=='default') {$optionsNext[$count] = $view->RenderOption(STATUS_OPERASI.STATUS_ANTRI,"Operasi Hari Ini",$show); $count++;}
     ...
     function GetTindakan($status) {
          ...
          for($i=0,$n=count($dataTable),$counter=0;$i<$n;$i++,$counter=0) {
			if($status==0) {
				...
				$tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$page[$dataTable[$i]["reg_status"]{0}].'?id_reg='.$dataTable[$i]["reg_id"].'"><img hspace="2" width="16" height="16" src="'.$APLICATION_ROOT.'images/b_select.png" alt="Proses" title="Proses" border="0"/></a>';               
			}/*&status='.$dataTable[$i]["reg_status"]{0}.'*/
     ...
     if($_GET["id_reg"]) {
		$sql = "select cust_usr_nama,cust_usr_kode, b.cust_usr_jenis_kelamin, a.reg_jenis_pasien,  
                    ((current_date - cust_usr_tanggal_lahir)/365) as umur,  a.id_cust_usr, c.ref_keluhan, a.reg_tipe_rawat 
	  ...
	  $sql_periksa = "select * from klinik.klinik_perawatan where id_reg=".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
	  $rs_periksa = $dtaccess->Execute($sql_periksa);
	  $dataPemeriksaan = $dtaccess->Fetch($rs_periksa);
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
		...
		if($_POST["btnSave"]) {
          ...
          $nextNya = $_POST["cmbNext1"];
		$sql = "update klinik.klinik_registrasi set reg_status = '".$nextNya."' where reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
		$dtaccess->Execute($sql); 
          ...
          $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["id_reg"]);
          $dbValue[2] = QuoteValue(DPE_CHAR,STATUS_OPERASI_JADWAL);
		...
		if($tglJadwal && $_POST["reg_jenis_pasien"]!=PASIEN_BAYAR_SWADAYA) {
          ...
          if($_POST["id_admin"])    {
               $sqlDelete = "delete from klinik.klinik_jadwal_admin where id_preop = ".QuoteValue(DPE_CHAR,$_POST["preop_id"]);
               $dtaccess->Execute($sqlDelete);
          
                $dbTable = "klinik_jadwal_admin";
                     
                $dbField[0] = "jadwal_admin_id";   // PK
                $dbField[1] = "id_preop";
                $dbField[2] = "id_pgw";
...
<?php $plx->Run(); ?>
//document.write("test js 0");
</script>

<script type="text/javascript">
     
function GantiRegulasi(tipe) {
     var paramnya;
     if(document.getElementById('preop_regulasi').checked) {
	  ...
	  document.getElementById('preop_regulasi_berhasil').checked = false;
	  paramnya = 'default';
     }
     SetNext1(paramnya,tipe,'target=td_target_next');
}

//document.write("test js 1");

function GantiBerhasil(tipe) {
     var paramnya;
...
//document.write("test js 2");

function CheckData(frm) {
     return true;
}

//document.write("test js 3");
...
//document.write("test js 5");

function SetDosis(id) {
     GetDosis(id,'target=sp_dosis');
}

//document.write("test js 6");
...
<table width="100%" border="0" cellpadding="4" cellspacing="1">
	<tr>
		<td align="left" colspan=2 class="tableheader">Input Data Penjadwalan</td>
	</tr>
</table> 
...
-->     
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
          ...
          <tr>
               ...
               <td align="left" class="tablecontent-odd" colspan="3">
                    ...
                    <?php echo $view->RenderTextBox("preop_tonometri_pressure_os","preop_tonometri_pressure_os","5","5",$_POST["preop_tonometri_pressure_os"],"inputField", "readonly",false);?> mmH
               </td>
          </tr>
          <tr class="subheader">
               <td align="left" ></td>
	...
	<!--
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
          <tr><!--'.$dataPasien["reg_tipe_rawat"].'-->
               <td align="left" width="20%" class="tablecontent">Perlu Regulasi</td>
               <td align="left" class="tablecontent-odd"><?php echo $view->RenderCheckBox("preop_regulasi","preop_regulasi","y","null",($_POST["preop_regulasi"] == "y")?"checked":"",'onClick="GantiRegulasi(\''.$dataPasien["reg_tipe_rawat"].'\');"')?>&nbsp;&nbsp;&nbsp;</td>
          </tr>
     </table>
     
     <table width="100%" border="1" cellpadding="4" cellspacing="1" id="tbRegulasi" style="display:none">
          <tr class="subheader">
          ...
          <tr><!--'.$dataPasien["reg_tipe_rawat"].'-->
               <td align="left" class="tablecontent-odd" colspan=4><label for="preop_regulasi_berhasil">Berhasil Diregulasi?</label><?php echo $view->RenderCheckBox("preop_regulasi_berhasil","preop_regulasi_berhasil","y","null",($_POST["preop_regulasi_berhasil"] == "y")?"checked":"",'onClick="GantiBerhasil(\''.$dataPasien["reg_tipe_rawat"].'\');"')?>&nbsp;&nbsp;&nbsp;</td>
          </tr>
     </table>

     <table width="100%" border="1" cellpadding="4" cellspacing="1" id="tbCatatan" style="display:block">
          <tr>
               ...
               <td align="left" class="tablecontent-odd" colspan=3>
				<select name="jadwal_jam" class="inputField" >
					<?php for($i=0,$n=24;$i<$n;$i++){ ?>
						<option class="inputField" value="<?php echo str_pad($i,2,'0',STR_PAD_LEFT);?>" <?php if($i==$_POST["jadwal_jam"]) echo "selected"; ?>><?php echo str_pad($i,2,'0',STR_PAD_LEFT);?></option>
					<?php } ?>
					</select>:
					<select name="jadwal_menit" class="inputField" >
					<?php for($i=0,$n=60;$i<$n;$i++){ ?>
						<option class="inputField" value="<?php echo str_pad($i,2,'0',STR_PAD_LEFT);?>" <?php if($i==$_POST["jadwal_menit"]) echo "selected"; ?>><?php echo str_pad($i,2,'0',STR_PAD_LEFT);?></option>
					<?php } ?>
				</select>
...
<input type="hidden" name="reg_tiper_rawat" value="<?php echo $_POST["reg_tipe_rawat"];?>" />
...
<script type="text/javascript">
    Calendar.setup({
     ...
     <?php if(!$_POST["preop_regulasi"] || $_POST["preop_regulasi"]=="n" ) { ?>    
	  document.getElementById('preop_regulasi').checked = false;
     <?php } ?>
     <?php if(!$_POST["preop_regulasi_berhasil"] || $_POST["preop_regulasi_berhasil"]=="n" ) { ?>
	  document.getElementById('preop_regulasi_berhasil').checked = false;
     <?php }?>
</script>
