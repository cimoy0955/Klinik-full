...
function GetData($in_nama,$in_kode){
	...
	#$sql_where[] = "icd_jenis = ".QuoteValue(DPE_CHAR,ICD_DIAGNOSIS); 
	if($in_nama) $sql_where[] = "UPPER(prosedur_nama) like ".QuoteValue(DPE_CHAR,strtoupper("%".$in_nama."%"));
	if($in_kode) $sql_where[] = "UPPER(prosedur_kode) like ".QuoteValue(DPE_CHAR,strtoupper("%".$in_kode."%"));
	if($sql_where) $sql_where = implode(" and ",$sql_where);

	// --- cari data krsnya ---
	$sql = "select prosedur_id, prosedur_kode, prosedur_nama from klinik.klinik_prosedur a";
	if($sql_where) $sql .= " where ".$sql_where;			
	$sql .= " order by a.prosedur_kode";
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["prosedur_id"].'\',\''.$dataTable[$i]["prosedur_kode"].'\',\''.addslashes(htmlentities($dataTable[$i]["prosedur_nama"])).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,kode,nama) {
	...
	self.parent.document.getElementById('rawat_prosedur_kode_<?php echo $_GET["el"];?>').value = kode;
	self.parent.document.getElementById('rawat_prosedur_nama_<?php echo $_GET["el"];?>').value = nama;
	self.parent.document.getElementById('rawat_prosedur_id_<?php echo $_GET["el"];?>').value = id;
...
<tr>
	<td>
		<table cellpadding="1" cellspacing="1" border="1" align="center" width="100%">
			...
			<tr>
				<td align="right" class="tablecontent" width="30%">No. Prosedur</td>
				<td class="tablecontent">
					<?php echo $view->RenderTextBox("_kode","_kode",30,200,$_POST["_kode"],false,false);?>
				</td>
			</tr>
			<tr>
				<td align="right" class="tablecontent" width="30%">Deskripsi</td>
				<td class="tablecontent">
