...
where id_app = ".QuoteValue(DPE_NUMERIC,'3');		
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
		...
		if($_POST["btnSave"]) {
          ...
          $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["id_reg"]);
          $dbValue[2] = QuoteValue(DPE_CHAR,STATUS_PREOP);
	  ...
	  //-- pasien ke diagnostik --//
          if($_POST["preop_regulasi"]=="n" && ($_POST["diag_keratometri"] || $_POST["diag_biometri"] || $_POST["diag_usg"] || $_POST["diag_lab_gula_darah"] || $_POST["diag_ekg"] || $_POST["diag_fundus"] || $_POST["diag_opthalmoscop"] || $_POST["diag_oct"] || $_POST["diag_yag"] || $_POST["diag_argon"] || $_POST["diag_glaukoma"] || $_POST["diag_humpre"] || $_POST["diag_slt"])){
	       $sql = "update klinik.klinik_registrasi set reg_status = '".STATUS_DIAGNOSTIK.STATUS_ANTRI."', reg_waktu = CURRENT_TIME  where reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
               ...
               // ---- inset ket folio ---//
               if($_POST["diag_keratometri"]) $sql_where[] = "biaya_id = ".QuoteValue(DPE_CHAR,BIAYA_KERATOMETRI);
               if($_POST["diag_biometri"]) $sql_where[] = "biaya_id = ".QuoteValue(DPE_CHAR,BIAYA_BIOMETRI);     
               if($_POST["diag_usg"]) $sql_where[] = "biaya_id = ".QuoteValue(DPE_CHAR,BIAYA_USG);
               if($_POST["diag_lab_gula_darah"]) $sql_where[] = "biaya_id = ".QuoteValue(DPE_CHAR,BIAYA_GULA);
               ...
               if($_POST["diag_slt"]) $sql_where[] = "biaya_id = ".QuoteValue(DPE_CHAR,BIAYA_SLT);
        ...
        $dbField[10] = "fol_nominal_satuan";
		 ...
		 // --- insrt suster ---
          ...
          if($_POST["id_admin"])    {
               $sqlDelete = "delete from klinik.klinik_preop_admin where id_preop = ".QuoteValue(DPE_CHAR,$_POST["preop_id"]);
          ...
          if($_POST["btnSave"]) {
	       if($_POST["cmbNext"]){
		    $sql = "update klinik.klinik_registrasi set reg_status = '".$_POST["cmbNext"].STATUS_ANTRI."' where reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
	       } else {
	...
	foreach($rawatKeadaan as $key => $value) {
     ...
     $sql = "select * from klinik.klinik_diagnostik where id_reg = ".QuoteValue(DPE_CHAR,$_GET["id_reg"]);
	...
	$optDosis[0] = $view->RenderOption("","[Pilih Dosis Obat]",$show); 
     ...
     //-- option combobox tahap berikutnya --//
     $count=0;	
     $optionsNext[$count] = $view->RenderOption(STATUS_DIAGNOSTIK_TIPE,"Ke Ruang Diagnostik",$show); $count++;
     $optionsNext[$count] = $view->RenderOption(STATUS_SELESAI,"Pulang Dengan Resep",$show); $count++;
     $optionsNext[$count] = $view->RenderOption(STATUS_OPERASI_JADWAL,"Jadwal Ulang",$show); $count++;
     $optionsNext[$count] = $view->RenderOption(STATUS_RAWATINAP,"Rawat Inap",$show); $count++;
     $optionsNext[$count] = $view->RenderOption(STATUS_SELESAI,"Ke Apotik dan selesai",$show); $count++;
     
     //-- combobox disimpan ke dalam variabel untuk dipanggil oleh javascript --//
     $comboNext = "Tahap Berikutnya&nbsp;".$view->RenderComboBox("cmbNext","cmbNext",$optionsNext,null,null,null)."&nbsp;&nbsp;&nbsp;";
?>
...
function GantiBerhasil(args) {
     if(args=="y") {
          document.getElementById('tbCatatan').style.display="block";
          document.getElementById('tbTerapi').style.display="none";
	  document.getElementById('tahap_berikutnya').innerHTML = '&nbsp;';
     } else if(args=="n"){
          document.getElementById('tbCatatan').style.display="none";
          document.getElementById('tbTerapi').style.display="block";
	  document.getElementById('tahap_berikutnya').innerHTML = '<?php echo preg_replace('/\n+/',' ',$comboNext);?>';
     }
}
...
<!--
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
	       <tr>
		    <td align="center"><span id="tahap_berikutnya"></span>
		    <?php echo $view->RenderButton(BTN_SUBMIT,($_x_mode == "Edit") ? "btnUpdate" : "btnSave","btnSave","Simpan","button",false,null);?></td>
	       </tr>
     </table>
     </fieldset>
     </td>
