<?php
	...
	$dtaccess = new DataAccess();
     ...
     $susterPage = "op_suster_find_1.php?";
    ...
    if(!$_POST["btnSave"] && !$_POST["btnUpdate"]) {
          ...
          $sql = "select distinct pgw_nama, pgw_id 
                  ...
                  where c.op_id = ".QuoteValue(DPE_CHAR,$_POST["op_id"]); 
...
$sql = "select pgw_nama, pgw_id from klinik.klinik_operasi_admin a
                    join hris.hris_pegawai b on a.id_pgw = b.pgw_id where id_op = ".QuoteValue(DPE_CHAR,$_POST["op_id"]);
          $rs = $dtaccess->Execute($sql);
...
where id_app = ".QuoteValue(DPE_NUMERIC,'4');		
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
		...
		$_POST["op_jam_selesai"] = $_POST["op_selesai_jam"].":".$_POST["op_selesai_menit"];
          ...
          if($_POST["id_admin"])    {
               $sqlDelete = "delete from klinik.klinik_operasi_admin where id_op = ".QuoteValue(DPE_CHAR,$_POST["op_id"]);
               $dtaccess->Execute($sqlDelete);
          
                $dbTable = "klinik_operasi_admin";
                     
                $dbField[0] = "operasi_admin_id";   // PK
                $dbField[1] = "id_op";
                $dbField[2] = "id_pgw";
  ...
  $sql = "update klinik.klinik_registrasi set reg_status = '".STATUS_PREMEDIKASI.STATUS_PROSES."'  where reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
 $dtaccess->Execute($sql);
  
  }
     $sql = "select preop_iol_jenis, preop_iol_merk from klinik.klinik_preop where id_reg = ".QuoteValue(DPE_CHAR,$_GET["id_reg"]);
     $dataIOLPreop = $dtaccess->Fetch($sql);
     if($dataIOLPreop){
          $_POST["op_iol_jenis"] = $dataIOLPreop["preop_iol_jenis"];
          $_POST["op_iol_merk"] = $dataIOLPreop["preop_iol_merk"];
     }

     $sql = "select preme_iol_jenis, preme_iol_merk from klinik.klinik_premedikasi where id_reg = ".QuoteValue(DPE_CHAR,$_GET["id_reg"]);
     $dataIOLPreme = $dtaccess->Fetch($sql);
     if($dataIOLPreme){
          $_POST["op_iol_jenis"] = $dataIOLPreme["preme_iol_jenis"];
          $_POST["op_iol_merk"] = $dataIOLPreme["preme_iol_merk"];
     }
     ...
     $sql = "select * from klinik.klinik_operasi_paket order by op_paket_nama";
     $dataOperasiPaket= $dtaccess->FetchAll($sql);

     // -- bikin combonya operasi paket
     $show = " style=\"font-family: monospace;\"";
...
<?php echo $view->RenderBody("inosoft.css",true,"onload=\"splitOptionLabel();\""); ?>
...
function splitOptionLabel(){
     var s = document.getElementsByName('op_paket_biaya')[0].options, 
          l = 0, 
          d = '';
...
function kosongiIna()
{
  document.getElementById('op_ina_kode').value="";
  document.getElementById('id_ina').value="";
  document.getElementById('op_ina_nama').value="";
}
...
function SetDisplay(id) {
     ...
     disp['none'] = 'inline-block';
     disp['inline-block'] = 'none';
...
<fieldset>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1">
	  ...
	  <tr>
	       <td align="left" width="20%" class="tablecontent">Paket Biaya</td>
               <td align="left" class="tablecontent-odd" width="30%" colspan="3"> 
          ...
          <tr>
		    ...
		    <table border="0" id="tbKompDurop" style="display:none;">
			 ...
			 <?php if($dataDurop[$i]["durop_komp_id"]==5) echo $view->RenderCheckBox("id_durop_komp[".$dataDurop[$i]["durop_komp_id"]."]","id_durop_komp_".$dataDurop[$i]["durop_komp_id"],"y","null",($_POST["id_durop_komp"][$dataDurop[$i]["durop_komp_id"]] == "y")?"checked":"",($dataDurop[$i]["durop_komp_id"]==5)?"onclick='SetDisplay(\"durop_komp_ket\")'":"")."<label for=\"id_durop_komp_".$dataDurop[$i]["durop_komp_id"]."\">".$dataDurop[$i]["durop_komp_nama"]."&nbsp;<input type=\"text\" name=\"durop_komp_ket\" id=\"durop_komp_ket\" width=\"20\" maxlength=\"255\" value=\"".$_POST["durop_komp_ket"]."\" class=\"inputField\" style=\"display:none;\" /> </label>&nbsp;";
			      else echo $view->RenderCheckBox("id_durop_komp[".$dataDurop[$i]["durop_komp_id"]."]","id_durop_komp_".$dataDurop[$i]["durop_komp_id"],"y","null",($_POST["id_durop_komp"][$dataDurop[$i]["durop_komp_id"]] == "y")?"checked":"",($dataDurop[$i]["durop_komp_id"]==5)?"onclick='SetDisplay(\"durop_komp_ket\")'":"")."<label for=\"id_durop_komp_".$dataDurop[$i]["durop_komp_id"]."\">".$dataDurop[$i]["durop_komp_nama"]." </label>&nbsp;";
                    } ?>
			      </td>
			 </tr>
			 <td align="left" class="tablecontent-odd" width="30%">Manajemen Komplikasi</td>
               <td align="left" class="tablecontent-odd"  > 
	  ...
	  <tr>
		   ...
		   <tr>
			  ...
			  <input type="text" size= "50" name="op_prosedur_nama[2]" id="op_prosedur_nama_2" value="<?php echo $_POST["op_prosedur_nama"][2]?>" readonly />
               ...
               <!--<td align="left" width="10%" class="tablecontent">INA DRG</td>
               <td align="left" class="tablecontent-odd" width="20%"> 
                    ...
                    &nbsp;
                    <input type="button" name="btnClear" id="btnClear" value="Kosongi" class="button" onClick="javascript: kosongiIna();" />
	       </td>-->
	  </tr>
	</table>
     </fieldset>
     ...
     <tr>
			<td align="center" colspan="2"><?php echo $view->RenderButton(BTN_SUBMIT,($_x_mode == "Edit") ? "btnUpdate" : "btnSave","btnSave","Simpan","button",false,"onClick=\"return CheckSimpan(this.form);\"");?>  <input type="submit" name="btnDel" id="btnDel" value="Batal" class="button" /></td>
