<?php 
	...
	if($_GET["id_reg"]) {
		$sql = "select cust_usr_nama,cust_usr_kode, b.cust_usr_jenis_kelamin, a.reg_jenis_pasien,  
                    ((current_date - cust_usr_tanggal_lahir)/365) as umur,  a.id_cust_usr, c.ref_keluhan, d.rawat_id 
		    ...
		    join klinik.klinik_perawatan d on a.reg_id = d.id_reg
                    where a.reg_id = ".QuoteValue(DPE_CHAR,$_GET["id_reg"]);
	       $rsPasien = $dtaccess->Execute($sql);
		...
		$sql = "select b.icd_nomor, b.icd_nama
				...
				where a.id_rawat = ".QuoteValue(DPE_CHAR,$dataPasien["rawat_id"])."
	...
	if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
		...
		if($_POST["btnSave"]) {
          ...
          unset($dbKey);
		   ...
		   $nextNya = ($_POST["preop_regulasi_berhasil"] == "n")?$_POST["cmbNext1"]:$_POST["cmbNext2"];
		$sql = "update klinik.klinik_registrasi set reg_status = '".$nextNya.STATUS_ANTRI."' where reg_id = ".QuoteValue(DPE_CHAR,$_POST["id_reg"]);
		$dtaccess->Execute($sql); 
...
*/
	...
	foreach($rawatKeadaan as $key => $value) {
     ...
     $optionsNext[$count] = $view->RenderOption(STATUS_SELESAI,"Apotek & Selesai",$show); $count++;
     ...
     $optionsNext1[$count] = $view->RenderOption(STATUS_SELESAI,"Apotek & Selesai",$show); $count++;
     $optionsNext1[$count] = $view->RenderOption(STATUS_RAWATINAP,"Rawat Inap",$show); $count++;
