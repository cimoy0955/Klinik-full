...
/* 
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		if($in_jenis=="12") $harganya = currency_format($dataTable[$i]["biaya_jual"]);
		else  $harganya = currency_format($dataTable[$i]["biaya_beli"]);
		...
		$tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$harganya;
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
		
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["biaya_id"].'\',\''.addslashes(htmlentities($dataTable[$i]["biaya_nama"])).'\',\''.($harganya).'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function Search() {
	...
	GetData(nama,kode,jenis,'target=dv_hasil');
}
		...
		<table cellpadding="1" cellspacing="1" border="1" align="center" width="100%">
			...
			<tr>
				<td colspan="2"><center>
					...
					<input type="hidden" name="id_jenis" id="id_jenis" value="<?php echo $_GET["jenis"];?>" />
				</td>
			</tr>
		</table>
	</td>
