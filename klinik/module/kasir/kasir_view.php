<?php
     ...
     function GetFolio() {
          ...
          $sql = "select distinct a.id_reg, a.id_cust_usr, b.cust_usr_nama,a.fol_jenis,a.fol_waktu,id_biaya,a.fol_lunas,b.cust_usr_jenis
                  ...
                  where a.fol_lunas = 'n' order by fol_waktu asc";
...
{
    if(!_wnd_new) {
			_wnd_new = window.open(url,judul,'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=500,height=700,top=35;left=150');
	} else {
		if (_wnd_new.closed) {
			_wnd_new = window.open(url,judul,'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=500,height=700,top=35;left=150');
		} else {
