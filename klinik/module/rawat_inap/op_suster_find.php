...
function GetData($in_nama){
	...
	$sql_where[] = "pgw_jenis_pegawai = ".PGW_JENIS_SUSTER; 
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$APLICATION_ROOT.'images/r_arrowgrnsm.gif" border="0" alt="Pilih" title="Pilih" width="12" height="12" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["pgw_id"].'\',\''.addslashes(htmlentities($dataTable[$i]["pgw_nama"])).'\',\''.$elnya.'\')"/>';
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
	}
...
function sendValue(id,nama,nomornya) {
	self.parent.document.getElementById('op_suster_terapi_nama_<? echo $elnya;?>').value = nama;
	self.parent.document.getElementById('id_suster_terapi_<? echo $elnya;?>').value = id;
